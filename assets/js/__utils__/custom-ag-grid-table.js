...
const initializeAgTable = (container, opt, obj) => {
  const agTable = (agGridTableContainer, options, obj) => {
    ...
    const updateBodyScrollDelay = -1; // 横向滚动延迟计时器
    ...
    options.floatingFilter = options && options.floatingFilter ? options.floatingFilter : true;
    // 是否显表头下方的浮动筛选框
    ...
    agTable.fixAgRenderChoke = () => {
      ...
      if (viewport) {
        const currentViewPortScrollLeft = agGridTableContainer.getAttribute('data-scroll-left');
        const currentViewPortScrollTop = agGridTableContainer.getAttribute('data-scroll-top');
    ...
    agTable.getSelect = () => api.getSelectedRows();
