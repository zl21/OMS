...
class BtnConfig {
    ...
    this.myconfig = {
      ...
      buttons: [
        ...
        },
        {
          ...
          btnclick: () => {
            commonUtils.navigateMain('-1', 'TabHref', 'returnTreasuryAdd', 'panel_label.returnTreasuryAdd')
          },
        },
        {
  ...
  forceMatchHandler(self, id) {
    if (BtnConfig.singleType) {
      commonUtils.serviceHandler(self, 'orderCenter.manualMatchingCheck', { id: id }, 'all', function (res) {
        if (res.data.code === 0) {
          commonUtils.navigateMain(id, 'TabOpen', 'WARNSENDMATCHING', 'panel_label.return_warehousing_wrong_delivery_forced_matching', { source: 3, form: 'list' });
        }
      });
    } else {
  ...
  manualMatchHandler(self, id) {
    if (BtnConfig.singleType) {
      commonUtils.serviceHandler(self, 'orderCenter.manualMatchingCheck', { id: id }, 'all', function (res) {
        if (res.data.code === 0) {
          commonUtils.navigateMain(id, 'TabOpen', 'manualMatching', 'panel_label.return_warehousing_manual_matching', { source: 2, form: 'list' });
        }
      });
    } else {
      let ids = this.orderStatusRule(self, {
  ...
  forcedCompletionHandler(self) {
    let ids = this.orderStatusRule(self, { type: 'check', statusCode: '30', statusTips: 'm8' });
    if (ids) {
      commonUtils.modalShow(self, 'm9', 'orderCenter.updateReturnBOrder', { ids });
    }
  }
  // 重传wms
  againWMSHandler(self) {
    let ids = this.orderStatusRule(self, { type: 'check', statusCode: '', statusTips: '' });
    if (ids) {
      commonUtils.serviceHandler(self, 'orderCenter.retransmissionWms', { returnOrderIds: ids.join(',') });
    }
  }
  // 重新生成订单
  async regenerateTheOrderHandler(self) {
    let ids = this.orderStatusRule(self, { type: 'radio', statusCode: '', statusTips: '' });
    if (ids) {
      const { data } = await self.service.orderCenter.validateRefundChange({ objid: ids[0] });
      if (data.code === 0) {
        commonUtils.navigateMain(id, 'TabOpen', 'RETURNGOOD', 'panel_label.ReturnOrderDetails', { statusName: true, flag: 'RefundToExchange' });
      } else {
  ...
  async cancelRefundHandler(self, id) {
    const { data } = await commonUtils.serviceHandler(self, 'orderCenter.cancelautorefund', { ID: id });
    if (data.code === 0) {
  ...
  async returnGoodsOrderHandler(self) {
    let ids = this.orderStatusRule(self, { type: 'radio', statusCode: '', statusTips: '' });
    if (ids) {
      const { data } = await self.service.orderCenter.refund2ExchangeValidate({ ids: ids });
      if (data.code === 0) {
        commonUtils.navigateMain(ids[0], 'TabHref', 'RETURNGOOD', 'panel_label.ReturnOrderDetails', { statusName: true, flag: 'RefundToExchange' })
      } else {
  ...
  modifyRemarkHandler(self, id) {
    self.changeRemarkConfig.componentData = { ids: id, status: self.statusTab, type: 1 };
    if (BtnConfig.singleType) {
  ...
  virtualStorageHandler(self, id) {
    if (BtnConfig.singleType) {
      commonUtils.modalShow(self, 'l7', 'common.updateVirtualLibrary', { ID: id });
    } else {
      let ids = this.orderStatusRule(self, {
        type: 'radio',
        statusCode: '20',
  ...
  returnGoodsCancelHandler(self, id) {
    if (BtnConfig.singleType) {
      commonUtils.serviceHandler(self, 'orderCenter.checkCancelParams', { ID: [id] });
    } else {
      let ids = this.orderStatusRule(self, { type: 'check', statusCode: '20', statusTips: 'l2', });
      if (ids) {
        commonUtils.serviceHandler(self, 'orderCenter.checkCancelParams', { ids });
      }
    }
  }
  //售后审核
  afterAuditOrderHandler(self, id) {
    if (BtnConfig.singleType) {
      ...
      if (ids) {
        ...
        commonUtils.serviceHandler(self, 'common.chargebackcheck', params, 'part', function (res) {
          if (res.data.code === 0) {
  ...
  auditOrderHandler(self, ids) {
    if (BtnConfig.singleType == 1) {
      ...
      commonUtils.serviceHandler(self, requestPath, { ids, type: '1' }, 'all', function (res) {
        if (res.data.code === 0) {
          self.$Message.success(res.data.message);
          self.autoRefresh();
        } else {
      ...
      commonUtils.serviceHandler(self, 'orderCenter.auditOrder', { ids, type: '1', isCheck: 0 });
    }
  }
  //反向审核订单处理
  reverseAuditOrderHandler(self, ids) {
    ...
    if (BtnConfig.singleType == 1) {
      commonUtils.modalShow(self, 'g7', 'orderCenter.auditOrderReserve', params, 'all', function (res) {
        if (res.data.code === 0) {
  ...
  invoiceNoticeHandler(self, fromdata) {
    self.service.orderCenter.checkAddOrderInvoicing(fromdata).then((res) => {
      if (res.data.code === 0) {
        ...
        commonUtils.navigateMain(-1, 'TabOpen', 'invoiceNoticetAdd', 'panel_label.billingNoticeEdit', extendObj);
      } else {
  ...
  invoiceRecordHandler(self, fromdata) {
    self.service.orderCenter.checkRecordInvoicing(fromdata).then((res) => {
      if (res.data.code === 0) {
        self.publicBouncedConfig = Object.assign(publicDialogConfig.makeOutInvoiceConfig, { componentData: res.data.data });
        setTimeout(() => {
          self.$children
  ...
  interceptOrderHandler(self, ids) {
    if (BtnConfig.singleType == 1) {
      commonUtils.serviceHandler(self, 'orderCenter.orderInterception', { ids }, 'all', function (res) {
        if (res.data.code === 0) {
          self.$Message.success(res.data.message);
          self.autoRefresh();
        }
  ...
  cancelInterceptionHandler(self, ids) {
    commonUtils.serviceHandler(self, 'orderCenter.cancelInterception', { ids: ids }, 'all', function (res) {
      if (res.data.code === 0) {
        self.$Message.success(res.data.message);
        self.autoRefresh();
      }
  ...
  scanIncomingHandler(self, ids) {
    commonUtils.serviceHandler(self, 'orderCenter.getScanIncomingInfo', { ids: ids }, 'all', function (res) {
      if (res.data.code === 0) {
        commonUtils.navigateMain(-1, 'TabOpen', 'scanIn', 'panel_label.scannAndWarehous', { returnId: recordID, isOrderHrefReturn: 'order' });
      } else {
      ...
      .then((res) => {
        ...
        if (res.data.code == 0) {
          ...
          if ([1, 2].includes(selectItem.ORDER_STATUS)) {
            commonUtils.navigateMain(selectItem.ID, 'TabHref', 'splitOrder', 'panel_label.orderSplit')
          } else {
  ...
  addReturnOrderHandler(self, selection) {
    if (selection.length > 1) {
      ...
      if ([7, 8, 1, 2, 3, 4, 50, 21].includes(orderDetails.ORDER_STATUS)) {
        ...
        let extendObj = { orderHrefReturnid: orderDetails.ID, isOrderHrefReturn: 'order' };
        commonUtils.navigateMain(-1, 'TabOpen', 'returngood', 'panel_label.addReturnOrder', extendObj);
      }
    }
  }
  //缺货重新占单处理
  outOfStockReOpeningHandler(self, ids) {
    if (BtnConfig.singleType) {
      commonUtils.serviceHandler(self, 'orderCenter.queryshortagSearchOrder', { ids }, 'all', function (res) {
        if (res.data.code === 0) {
          self.$Message.success(res.data.message);
          self.autoRefresh();
        }
      ...
      commonUtils.serviceHandler(self, 'orderCenter.queryshortagSearchOrder', { ids });
    }
  }
  ...
  fortuneBagReOpeningHandler(self, ids) {
    if (BtnConfig.singleType) {
      commonUtils.serviceHandler(self, 'orderCenter.queryFortuneBagShortage', { ids }, 'all', function (res) {
        if (res.data.code === 0) {
          self.$Message.success(res.data.message);
          self.autoRefresh();
        }
      ...
      commonUtils.serviceHandler(self, 'orderCenter.queryFortuneBagShortage', { ids });
    }
  }
  //备注导入处理
  noticeImport() {
  ...
  changeShipmentPlatformHandler(self, ids) {
    commonUtils.serviceHandler(self, 'orderCenter.doManualDeliveryOrder', { ids });
  }
  //批量退单处理
  ...
  refundPriceHandler(self, data) {
    if ([5, 6].includes(data.orderStatus)) {
      ...
      commonUtils.navigateMain(-1, 'TabOpen', 'EXTRAREFUND', 'panel_label.extraRefundEdit', extendObj);
    } else {
  ...
  orderUpdateAddressHandler(self, selection) {
    ...
    commonUtils.serviceHandler(self, 'orderCenter.getDetail', params, 'all', function (res) {
      if (res.data && res.data.code === 0) {
        const order = res.data.data;
        let tempObj = (({
          BUYER_MESSAGE, SELLER_MEMO, SYSREMARK, CP_C_REGION_PROVINCE_ID, CP_C_REGION_CITY_ID, CP_C_REGION_AREA_ID,
          RECEIVER_ADDRESS, RECEIVER_NAME, RECEIVER_MOBILE, RECEIVER_PHONE, RECEIVER_ZIP }) => ({
            BUYER_MESSAGE, SELLER_MEMO, SYSREMARK, CP_C_REGION_PROVINCE_ID, CP_C_REGION_CITY_ID, CP_C_REGION_AREA_ID,
            RECEIVER_ADDRESS, RECEIVER_NAME, RECEIVER_MOBILE, RECEIVER_PHONE, RECEIVER_ZIP
          }))(order);
  ...
  updateStoreHandler(self, data) {
    ...
    commonUtils.serviceHandler(self, 'orderCenter.checkOrderBeforeWarehouse', fromdata, 'all', function (res) {
      if (res.data.code === 0) {
  ...
  revisingUpdateHandler(self, data) {
    let tempObj = { ids: [data.ID], cLogisticsId: 0, platform: data.PLATFORM, CP_C_PHY_WAREHOUSE_ID: data.CP_C_PHY_WAREHOUSE_ID };
    const fromdata = new FormData();
    fromdata.append('ids', tempObj.ids);
    commonUtils.serviceHandler(self, 'orderCenter.checkOrderBeforeLogistics', fromdata, 'all', function (res) {
      if (res.data.code === 0) {
