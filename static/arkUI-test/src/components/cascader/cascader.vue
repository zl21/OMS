...
<script>
    ...
    const prefixCls = `${burgeonConfig.prefixCls}cascader`;
    const selectPrefixCls = `${burgeonConfig.prefixCls}select`;
        ...
        computed: {
            ...
            displayRender () {
                ...
                for (let i = 0; i < this.selected.length; i++) {
                    label.push(this.selected[i][this.labelName]);
                }
...
return false;
                ...
                const oldVal = JSON.stringify(this.currentValue);
                this.currentValue = item[this.valueName].split(',');
                this.emitValue(this.currentValue, oldVal);
                this.handleClose();
            },
            handleFocus () {
        ...
        created () {
            ...
            this.$on('on-result-change', (params) => {
                ...
                const {changeOnSelect} = params;
                const {fromInit} = params;
                ...
                if (lastValue || changeOnSelect) {
                    ...
                    this.selected.forEach((item) => {
                        newVal.push(item[this.valueName]);
                    });
