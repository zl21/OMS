...
const prefixCls = `${burgeonConfig.prefixCls}checkbox`;
...
export default {
    ...
    wrapClasses() {
      return [
        `${prefixCls}-wrapper`,
        {
          ...
          [`${prefixCls}-wrapper-circle`]: !!this.circle
        }
      ];
    },
    checkboxClasses() {
  ...
  updated() {
    ...
    if (
      customData
      && Object.prototype.toString.call(customData) === "[object Object]"
    ) {
  ...
  methods: {
    change(event) {
      ...
      if (this.group) {
        ...
        this.$emit("on-change", value, this);
