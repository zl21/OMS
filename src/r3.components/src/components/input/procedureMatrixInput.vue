<template>
  ...
  >
    <div
      ...
      @click.stop=""
    >
      <span>{{$t('input.productNumber')}}︰</span>
      <el-popover
        ref="popover"
        v-model="visible"
        placement="bottom"
      ...
      >
        ...
        >
          ...
          <input
            ...
            :placeholder="$t('input.enterBarcode')"
  ...
  export default {
          ...
          if (data.TYPE === 'proEcode') { // 商品编码
            ...
            if (this.objid == -1) {
              if (index !== -1) {
                ...
                const QTY_BILL = this.inputList.findIndex(n => n.colname === 'QTY_BILL');
                if (QTY_BILL !== -1) {
                  this.$set(this.inputList[QTY_BILL], 'valuedata', this.count === '' ? this.matrixnum : this.count);
                  this.$emit('getChangeItem', this.inputList[QTY_BILL]);
                }
    ...
    watch: {
      ...
      search(val, oldval) {
        ...
        this.search = val.toLocaleUpperCase();
        ...
        if (val && this.isCustom) {
          ...
          if (Object.keys(this.singleData).length !== 0 && this.judgeSingle) {
            axios({
              ...
              if (resData.code === 0) {
                this.$message({
                  message: this.$t('input.addBarcode'),
                  center: true,
                  type: 'success'
                });
                this.search = '';
