<template>
  ...
  >
    ...
    >
      <span>{{$t('input.productNumber')}}︰{{ encode }}</span>
      <span
        v-if="hasStock"
      ...
      >
        ...
        <button
          ...
          @click="matchStock"
        >
          <span>{{$t('custompage.matchInLib')}}</span>
        </button>
        <button
          class="ff-matrix-btn ff-matrix-btn-cancel"
          @click="matchUsable"
        >
          <span>{{$t('custompage.matchAvailable')}}</span>
        </button>
      </span>
      <span
        v-else
    ...
    >
      <span>
        {{ tablename === 'DL_B_RETAIL'? $t('custompage.shop'): $t('input.store') }}︰{{ store }}
        <label class="ff-matrix--active-one ff-matrix--active-label">
          <input
            v-model="stock"
            type="checkbox"
          > {{ hasReceiving? $t('custompage.shipInStore') : $t('custompage.inStore') }}
        </label>
        <label class="ff-matrix--active-two ff-matrix--active-label">
          <input
            v-model="usable"
            type="checkbox"
          > {{ hasReceiving? $t('custompage.shipmentAvailable') : $t('custompage.available') }}
        </label>
        <label
          v-if="hasReceiving"
        ...
        >
          <input
            ...
            type="checkbox"
          > {{$t('custompage.receiptAvailable')}}
        </label>
        {{ hasReceiving? '' : `（ ${$t('custompage.inventoryStatus')} ）` }}
      </span>
      <span
        class="ff-matrix--clear"
        @click="empty"
          ...
          <tr class="ff-matrix--edit-color">
            <th class="ff-matrix-th-head">
              <div class="ff-matrix-th-head-box">
                <span>{{$t('tips.color')}}</span>
                <span>{{$t('tips.size')}}</span>
              </div>
            </th>
              ...
              <ul v-if="edit || general">
                ...
                >
                  ...
                  >
                    ...
                    <span>{{$t('custompage.enterDigit',{num:4})}}</span>
                  </div>
                </li>
              </ul>
            </td>
    ...
    >
      ...
      <span>{{$t('custompage.barcodeNotExist')}}</span>
    </div>
    <div
      v-if="general"
...
<style lang="less" scoped type="text/less">
  ...
  .ff-matrix--table {
    ...
    .ff-matrix--box-btn, .ff-matrix--reveal, .ff-matrix--matching {
      text-align: right;
      .ff-matrix-btn {
        padding: 6px 21px;
        border-radius: 2px;
      ...
      .ff-matrix-btn-confirm {
        ...
        color: #fff;
        &:hover {
          background-color: rgba(253, 100, 66, .7);
        }
      }
  ...
  export default {
    ...
    data() {
      return {
        ...
        store: this.$t('custompage.hangzhouStore'), // 店仓名字
    ...
    methods: {
      ...
      matchStock() {
        ...
        this.$message({
          message: this.$t('custompage.matchSuccessfully'),
          type: 'success'
        });
      ...
      countChange(event, key, list, val) {
        ...
        const isNum = (this.tablename !== 'DL_B_PAND' && this.tablename !== 'DL_B_INV_ADJ_WORK_PICK_POS' && this.tablename !== 'DL_B_INV_ADJ_WORK_PICK' && this.tablename !== 'DL_B_PUR_REQ' && this.tablename !== 'DL_B_PUR_ORDER' && this.tablename !== 'DL_B_PUR_TMPIN') ? /^-?[0-9]*$/.test(value) : /^[0-9]*$/.test(value);
      ...
      waitObjSave() {
        axios({
          ...
          if (resData.code === 0) {
            this.$message({
              message: this.$t('feedback.saveSuccess'),
              type: 'success'
            });
          } else {
            let errorData;
            if (resData.data) {
              ...
              this.$store.commit('customize/errorDialog', { // 其它报错
          ...
          this.tHead.map((data, sub) => {
            if (this.reveal) { // 纯展示
              ...
              this.tBody[obj.name][data.name] = Object.assign({}, {
                /* stock: stock === undefined? 0: stock['qty_stock'],//库存
                usable: stock === undefined? 0: stock['qty_stock_able'],//可用 */
              ...
              if (skuObj === undefined) {
                ...
                this.tBody[obj.name][data.name] = Object.assign({
                  ...
                  disabled: true, // 不可编辑
                  stock: this.hasStock ? (stock === undefined ? 0 : stock.qty_stock) : 0, // 库存
                  usable: this.hasStock ? (stock === undefined ? 0 : stock.qty_stock_able) : 0, // 可用
                  stockReceiving: this.hasReceiving ? (receivingData === undefined ? 0 : receivingData.qty_stock) : 0, // 收货在库
                  usableReceiving: this.hasReceiving ? (receivingData === undefined ? 0 : receivingData.qty_stock_able) : 0, // 收货可用
                  count: valObj === undefined ? '' : valObj.MATRIX_DATA.QTY_BILL, // 数量
                  TAG: `${index}-${sub}`// 唯一标识符
                });
                this.isFooter = true;
              } else {
                ...
                /* 收货在用库存找出和这个尺寸对应的sku */
                const receivingData = this.hasReceiving ? this.receivingData.find(n => n.ps_c_clr_id === obj.id && n.ps_c_size_id === data.id) : {};
                this.tBody[obj.name][data.name] = valObj === undefined ? Object.assign({}, skuObj, {
                  stock: this.hasStock ? (stock === undefined ? 0 : stock.qty_stock) : 0, // 在库
                  usable: this.hasStock ? (stock === undefined ? 0 : stock.qty_stock_able) : 0, // 可用
                  stockReceiving: this.hasReceiving ? (receivingData === undefined ? 0 : receivingData.qty_stock) : 0, // 收货在库
                  usableReceiving: this.hasReceiving ? (receivingData === undefined ? 0 : receivingData.qty_stock_able) : 0, // 收货可用
                  count: '', // 数量
                  ...
                  TAG: `${index}-${sub}`, // 唯一标识符
                  control: false, // 控制提示框
                  count: valObj.MATRIX_DATA.QTY_BILL, // 可编辑(通用都为空，只有纯展示和编辑的时候用后台返回的)
                  disabled: false, // true: 表示不可编辑 false:表示可编辑
                });
              }
            }
      ...
      getTableData() {
        this.getHeadData().then(() => { // 获取头部信息
          ...
          // let data = res.data;
          const data = { code: 0, data: [], message: 'success' }; // 因为不需要矩阵框中查询数量,故值写死为零
          if (data.code === 0) {
