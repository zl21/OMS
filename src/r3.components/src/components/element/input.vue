      ...
      if (self.itemdata.fkdisplay == 'mop') {
        if (self.itemdata.reftable === 'VP_C_VIP_ACC') {
          if (self.itemdata.valuedata) {
            ...
            self.itemdata.valuedata = this.$t('messages.selectedData',{total:JSON.parse(self.mopDefaultValue).count});
          } else {
            self.fkDialog.lists = {};
          }
          ...
          self.fkDialog.lists = self.mopDefaultValue;
          self.itemdata.pid = self.itemdata.valuedata;
          self.itemdata.valuedata = this.$t('messages.selectedData',{total:JSON.parse(self.mopDefaultValue).total});
        } else {
          self.fkDialog.lists = {};
        }
      }
        ...
        handler(val, oldVal) {
          ...
          if (self.itemdata.fkdisplay == 'mop') {
            ...
            if (
              ...
              self.itemdata.pid = self.itemdata.valuedata;
              self.fkDialog.lists = self.mopDefaultValue;
              self.itemdata.valuedata = this.$t('messages.selectedData',{total:self.itemdata.reftable === 'VP_C_VIP_ACC'
    ...
    methods: {
          ...
          self.objList.forEach((item) => {
            if (!findflag && item.childs) {
              item.childs.forEach((child) => {
                if (child.colname === itemdata.refcolprem.srccol) {
                  ...
                  if (!itemdata.refcolprem.refcolval) {
                    self.$message({
                      message: `${this.$t('form.selectPlaceholder')}${child.name}`,
                      type: 'warning'
                    });
              ...
              if (item.child.colname === itemdata.refcolprem.srccol) {
                ...
                if (!itemdata.refcolprem.refcolval) {
                  self.$message({
                    message: `${this.$t('form.selectPlaceholder')}${item.child.name}`,
                    type: 'warning'
                  });
      ...
      advancedData(val) {
        ...
        if (val) {
          ...
          self.itemdata.valuedata = this.$t('messages.selectedData',{total:JSON.parse(val).count});
      ...
      getFkdialog(item) {
        ...
        if (ITEM.lists.result.length > 0) {
          ...
          self.itemdata.valuedata = this.$t('messages.selectedData',{total:ITEM.total});
