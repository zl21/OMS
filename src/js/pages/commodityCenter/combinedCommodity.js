...
export default {
  ...
  methods: {
    ...
    inputCapital(type) {
      const _this = this;
      if (type === 1) { _this.formConfig1.formValue.ECODE = _this.formConfig1.formValue.ECODE.toUpperCase(); } else if (type === 2) { _this.formConfig1.formValue.ENAME = _this.formConfig1.formValue.ENAME.toUpperCase(); } else if (type === 3) { _this.jordanTableConfig1.jordanFormConfig.formValue.ECODE = _this.jordanTableConfig1.jordanFormConfig.formValue.ECODE.toUpperCase(); } else if (type === 4) { _this.jordanTableConfig1.jordanFormConfig.formValue.PS_C_PRO_ENAME = _this.jordanTableConfig1.jordanFormConfig.formValue.PS_C_PRO_ENAME.toUpperCase(); }
    },
    // 查找商品编码的
    ...
    selecttype(type) {
      if (type === 2) {
        ...
        // 表头也要加入该字段
        const isGROUP_EXTRACT_NUM = this.jordanTableConfig1.columns.every(
          ele => ele.key !== 'GROUP_EXTRACT_NUM'
        );
      ...
      if (this.jordanTableConfig1.data.length > 0) {
        ...
        for (const item of this.jordanTableConfig1.data) {
          if (item.ECODE === inputData.ECODE) {
            ...
            const info = vmI18n.t('modalTips.w7')
              + inputData.ECODE
    ...
    comodityDetailEvent() {
      ...
      if (this.jordanTableConfig1.data[this.clickIndex].isActive) {
        if (
          ...
          const flag = this.jordanTableConfig1.data[
            this.clickIndex
          ...
          if (flag !== -1) {
            ...
            const itemI = this.jordanTableConfig1.data[this.clickIndex]
              .psCSkugroupList[flag];
            const targetnum = this.jordanTableConfig1.data[this.clickIndex]
              .psCSkugroupList[flag].NUM;
            const num = (parseInt(targetnum) + parseInt(t_data.NUM)).toString(); // 相同条码的数量进行合并
