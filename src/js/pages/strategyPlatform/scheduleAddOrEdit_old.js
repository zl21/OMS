    ...
    return {
      ...
      pickingForStatus: {
        ...
        formValue: {
          ...
          PEAK_VALUE: '', // 创建峰值
          AUTO_PICK_INTERVAL_TIME: '2', // 自动拣货间隔时间
          PICKORDER_TYPE: ['1'], // 拣货单类型
          CP_C_ORIG_ID: '' // 唯品会仓库ID
        },
        ruleValidate: {
      ...
      warehouseWarrant: [
        {
          ...
          formConfig: {
            formData: [{
              ...
              disabled: false,
              tips: {
                content: $it('tip.hw') // 当“店铺+唯品会仓库+发货仓库”相同的配货单中商品数量大于等于设置的峰值时，自动创建入库单
              },
              inputChange: () => {
        ...
        },
        {
          ...
          formConfig: {
            formData: [{
              ...
              disabled: false,
              tips: {
                content: $it('tip.hv') // 每天在该时间点自动对创建的入库单结单
              },
              onChange: () => {
          ...
          formData: [
            {
              ...
              options: [ // radio选项
                {
                  value: '1',
                  label: $it('pL.ap'), // 按时间点创建
                },
                {
                  value: '2',
                  label: $it('pL.ab'), // 按未拣货数创建
                }
              ]
            }
          ],
      ...
      pickTableHeaders: [
        ...
        {
          title: $it('fL.bs'), // 唯品会仓库
          key: 'CP_C_ORIG_ENAME',
        },
        {
          title: $it('fL.bt'), // 拣货单类型
          key: 'PICKORDER_TYPE',
          render: (h, params) => {
            let types = {
              1: $it('other.common'), // 普通
              2: $it('fL.bx'), // 补货单 
              // 3: $it('fL.by'), // 预调拨
            }
        ...
        {
          title: $it('fL.bw'), // 自动拣货间隔时间
          key: 'AUTO_PICK_INTERVAL_TIME',
          render: (h, params) => {
            return  h('span', `${params.row.AUTO_PICK_INTERVAL_TIME}h`)
          }
        },
        {
          title: $it('fL.bs'), // 唯品会仓库
      ...
      warehouseWarrantConfig: {
        ...
        columns: [
          ...
          {
            title: $it('fL.distributionMode'), // 配送方式
            key: 'DELIVERY_WAY',
            render: (h, params) => {
              let types = {
                '1': $it('fL.cb'), // 汽运
      ...
      logConfig: {
        ...
        columns: [
          ...
          {
            title: $it('tL.before_modification'), // 修改前('pH.userName'), // 用户名
  ...
  computed: {
    dialogInfo() {
      return this.curDialog == 'warehouseWarrant'
        ? { title: $it('pL.ac'), width: '800' }
        : { title: $it('pL.ad'), width: '400' };
    }
  },
  methods: {
    // 时间戳格式化
    ...
    */
    ...
    async querySchedule() {
      ...
      if (!data) return
      this.formConfig = $utils.initFormConfig(data.addcolums[0].childs, this.formConfig);
    ...
    save() {
      ...
      const mes = $utils.validatorNotEmpty(self.formConfig, valueArr, drpArr);
      if (mes !== 1) {
      ...
      .then(({ data: { data } }) => {
        ...
        this.$message.error($it('tip.y0')); // 保存失败
      });
    },
    /**
     * 拣货单/入库单保存
