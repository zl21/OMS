...
export default {
  ...
  methods: {
    ...
    getAutoCheck() {
      ...
      return this.service.strategyPlatform.getAutoCheck(params)
      // return httpFormdata({
      //   method: 'post',
      ...
      // })
        .then(({ data }) => {
          if (data.data.code == 0) {
            ...
            this.info.beginTime = this.info.BEGIN_TIME
              ? timestampToTime(this.info.BEGIN_TIME)
              : '';
            this.info.endTime = this.info.END_TIME
              ? timestampToTime(this.info.END_TIME)
              : '';
            // this.info.CP_C_REGION_PROVINCE_ENAME =
            // this.effectiveCondition = this.info.EFFECTIVE_CONDITION
            //   ? this.info.EFFECTIVE_CONDITION.split(",")
            //   : [];
            const arr = this.info.EFFECTIVE_CONDITION
              ? this.info.EFFECTIVE_CONDITION.split(',')
              : [];
            this.info.CP_C_REGION_PROVINCE_ENAME ? this.info.CP_C_REGION_PROVINCE_ENAME.split(',').map((item) => {
              const tmp = {
                ID: '-1',
                Label: item,
              };
              this.providesList.push(tmp);
            }) : '';
            ...
            this.CREATIONDATE = this.info.CREATIONDATE
              ? timestampToTime(this.info.CREATIONDATE)
              : '';
            this.MODIFIEDDATE = this.info.MODIFIEDDATE
              ? timestampToTime(this.info.MODIFIEDDATE)
              : '';
            return;
          }
    ...
    QueryList() {
      ...
      this.service.strategyPlatform.QueryList(params)
      // httpFormdata({
      //   method: 'post',
      //   url: '/p/cs/QueryList',
      //   headers: {
