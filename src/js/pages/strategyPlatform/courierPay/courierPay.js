...
export default {
  ...
  methods: {
    ...
    setData(data) {
      ...
      formValue_one.CREATIONDATE = self.setTime(
        data.ST_C_COMPENSATE.CREATIONDATE
      ); // 创建时间
      formValue_one.MODIFIERENAME = data.ST_C_COMPENSATE.MODIFIERENAME; // 修改人
      formValue_one.MODIFIEDDATE = self.setTime(
        data.ST_C_COMPENSATE.MODIFIEDDATE
      ); // 修改时间
    ...
    getData(val = -1) {
      ...
      const fixcolumn = {};
      const objid = val;
      const ST_C_COMPENSATE = JSON.parse(
        JSON.stringify(self.formConfig.formValue)
      ); // 主表数据
