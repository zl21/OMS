  ...
  export default {
    ...
    methods: {
      ...
      save() {
        ...
        if (_this.$route.params.customizedModuleId === 'New') {
          if (!_this.information.formValue.ENAME) return _this.$Message.error('规则名称必填');
          if (!ETYPE) return _this.$Message.error('规则类型必填');
        }
        if (!_this.theadArr.length && ETYPE === '1' && _this.$route.params.customizedModuleId !== 'New') return _this.$Message.error('请先设置仓库');
        ...
        this.service.strategyPlatform.saveSendRule(formData)
        // axios({
      ...
      saveWherehouseAdd() {
        ...
        if (!_this.jordanTableConfig.jordanFormConfig.formData[0].itemdata.pid) return _this.$Message.error('仓库必填');
        if (!_this.information.formValue.ETYPE) return _this.$Message.error('仓库优先级必填必填');
      ...
      getMianTable() {
        const _this = this;
        _this.saveLoading = true;
        this.service.strategyPlatform.getSendRuleMain({ objid: _this.$route.params.customizedModuleId == 'New' ? '-1' : _this.$route.params.customizedModuleId })
        // axios({
      ...
      getWarehouseRateResult() {
        ...
        this.service.strategyPlatform.getWarehouseRateResultTable({ objid: _this.$route.params.customizedModuleId == 'New' ? '-1' : _this.$route.params.customizedModuleId })
        // axios({
      ...
      enter(e) {
        ...
        this.service.strategyPlatform.getLikeRankResultTable(param)
        // axios({
        ...
        .then(res => {
          ...
          if (res.data.code === 0) {
            ...
            _this.listArr.forEach(item => item.WAREHOUSE_RANK = JSON.parse(item.WAREHOUSE_RANK));
      ...
      invalid() {
        ...
        this.service.strategyPlatform.voidSendRule(fromdata)
        // axios({
      ...
      tableExport() {
        ...
        this.service.strategyPlatform.exportSendRuleWarehouseRate(param)
        // axios({
      ...
      DeleteDetail() {
        ...
        if (_this.selectAllList.length) {
          ...
          return _this.$Message.error('请选择需要删除的数据');
        }
        const fromdata = new FormData();
        const param = {
          objid: _this.$route.params.customizedModuleId == 'New' ? '-1' : _this.$route.params.customizedModuleId,
        ...
        fromdata.append('param', JSON.stringify(param));
        this.service.strategyPlatform.delSendRule(fromdata)
        // axios({
      ...
      warningOk() {
        ...
        this.service.strategyPlatform.exportSendRuleWarehouseRank(param)
        // axios({
