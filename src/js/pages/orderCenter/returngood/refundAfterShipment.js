...
export default {
    ...
    return {
      ...
      order: {
        ...
        orderform: {
          ...
          formData: [
            ...
            {
              style: 'input',
              label: $it('tL.platform_orderNo'), // 平台订单号
              value: 'source_code',
              width: '8',
              inputenter: () => this.queryBounced()
            },
            ...
            },
            {
              ...
              itemdata: {
                ...
                name: $it('tL.shopName'), // 店铺名称input前面显示的lable值
        ...
        table: {
          columns: [
            ...
            },
            {
              ...
              title: $it('fL.delivery_warehouse') // 发货仓库
            },
            // {
            //   key: "PLATFORM",
            //   title: "平台状态"
      ...
      reForm: {
        ...
        config: [
          ...
          },
          {
            item: {
              type: 'Input',
              colname: 'sourceDocuments',
              label: $it('fL.sourceDocuments'), // 单据来源
              props: {
                value: '',
                disabled: true
              }
          ...
          // },
          {
            item: {
              type: 'Input',
              colname: 'ID',
              label: $it('fL.originalOrderNo'), // 原始订单编号
              required: true,
              props: {
                icon: 'ios-search',
                value: ''
          ...
          //     type: 'Input',
          // colname: 'SOURCE_CODE',
          //     label: $it('fL.originalOrderNo'), // 原始平台单号
          //     required: true,
          //     props: {
          //       value: '',
          //       disabled: true,
          ...
          // },
          {
            item: {
              type: 'Input',
              // colname: 'CP_C_SHOP_TITLE',
              label: $it('fL.originalPlatformNo'), // 平台退款单号
              // required: true,
              props: {
                value: ''
              },
          ...
          },
          {
            item: {
              type: 'Input',
              colname: 'VIP_PHONE',
              label: $it('fL.BuyerPhoneNumber'), // 买家手机号
              // required: true,
              props: {
                value: '',
                disabled: false
          ...
          },
          {
            item: {
              type: 'DatePicker',
              colname: 'documentDate',
              label: $it('fL.documentDate'), // 单据日期
              required: true,
              props: {
                value: ''
              }
          ...
          },
          {
            item: {
              type: 'Input',
              colname: 'RETURNABLE_AMOUNT',
              label: $it('fL.refundAmount'), // 申请退款金额
              props: {
                value: '',
                disabled: true
              },
          ...
          },
          {
            item: {
              type: 'Select',
              label: $it('fL.responsibleParty'), // 判责方
              props: {
                value: '',
                options: []
              },
          ...
          },
          {
            item: {
              type: 'Input',
              label: $it('fL.remarksResponsibleParty'), // 判责方备注
              props: {
                value: ''
              },
              event: {}
            ...
            btnclick: () => {
              ...
              if (this.$route.params.customizedModuleName === 'EXTRAREFUND') {
                R3.store.commit('global/tabOpen', {
                  ...
                  back: true,
                  label: $it('btn.additionalRefund') // 额外退款
                });
                ...
                R3.store.commit('global/tabOpen', {
                  ...
                  back: true,
                  label: $it('fL.refundNoteDelivered') // 已发货退款单
                });
              }
              // 销毁当前实例
              this.$destroy();
      ...
      tableConfig: {
        ...
        columns: [
          ...
          // },
          {
            title: $it('fL.platformProductsCode'), // 商品编码
            key: 'ecode'
          },
          {
            title: $it('fL.platformProductsName'), // 平台商品名称
            key: 'PT_PRO_NAME'
          },
          {
          ...
          {
            title: $it('fL.transactionAmount'), // 成交金额
            key: 'realAmt'
          },
          {
            title: $it('fL.returnQuantity'), // 退货数量
            key: 'QTY_IN',
            render: (h, params) => {
              const self = this;
              return h(
                    ...
                    on: {
                      'on-change': e => {
                        ...
                        self.reForm.config.find(
                          item => item.item.label == $it('fL.refundAmount') // 申请退款金额
      ...
      if (this.$route.query.fromOrder) {
        ...
        this.reForm.config.splice(14, 0, {
          item: {
            type: 'Input',
            colname: 'AMT_RETURN',
            label: $it('fL.actualRefundAmount'), // 实际退款金额
            props: {
              value: '',
  ...
  methods: {
    changeForm() {
      this.$nextTick(() => {
        this.reForm.config.forEach((val, index) => {
          if (val.item.colname === 'sourceDocuments') {
            this.reForm.config[index].item.props.value = '手动';
          } else if (val.item.colname === 'documentDate') {
    ...
    async fromOrder(listData, initStatus = false) {
      ...
      const setVal = ['ID', 'CP_C_SHOP_TITLE', 'SOURCE_CODE', 'USER_NICK', 'VIP_PHONE', 'RECEIVER_NAME'];
      _this.reForm.config.forEach(item => {
        if (setVal.includes(item.item.colname)) {
          item.item.props.value = data[item.item.colname]
        } else {
          switch (item.item.colname) {
            case 'PAY_TYPE':
              item.item.props.value = String(data.PAY_TYPE) || '1';
              break;
            case 'RETURNABLE_AMOUNT':
        ...
        // 收款人姓名
        if (item.item.label === $it('fL.namePayee')) item.item.props.value = data.RECEIVER_NAME;
    ...
    queryorder(listData, isEnter) {
      ...
      if (isEnter) {
        ...
        if (this.order.table.selectArr.length != 1) {
          _this.$Message.warning($it('tip.i7')); // 请选择一条数据！
          return false;
        }
    ...
    threeObjs() {
      ...
      _this.order.orderform.formData.forEach(item => {
        // 店铺名称
        if (item && item.colname === 'cp_c_store') {
    ...
    save() {
      ...
      this.service.orderCenter.saveAfterDeliver(data).then(res => {
        if (res.data.code == 0) {
          ...
          self.$Message.error(res.data.message || 'save error !');
        }
      });
    },
    audit() {
    ...
    isNull() {
      ...
      if (!flag) {
        if (!this.returnTypeFormConfig.formValue.OC_B_RETURN_TYPE_ID) {
          ...
          flag = {
            item: { label: `${$it('fL.refundDescription')}` }
          };
        }
      }
    ...
    tableAddDetail() {
      ...
      for (let i = 0; i < this.reForm.config.length; i++) {
        if (self.reForm.config[i].item.colname == 'ID') {
        ...
        if (res.data.code == 0) {
          ...
          self.addItem.table.data.forEach(item => {
            ...
            switch (item.GIFT_TYPE) {
              case 1:
                tempStr = $it('fL.eb'); // 系统赠品
                break;
              case 2:
                tempStr = $it('fL.ec'); // 平台赠品
                break;
              default:
                tempStr = $it('com.no'); // 否
            }
      ...
      if (self.$route.params.customizedModuleId === 'New' || self.$route.query.cid || self.$route.query.new === 'true') {
        if (!self.isOne) {
          ...
          self.reForm.config.forEach(item => {
            ...
            // 收款人姓名;
            if (configItemLabel === $it('fL.namePayee')) configItem.props.value = data.RECEIVER_NAME;
        ...
        data.orderId = this.reForm.config.filter(item => item.item.label == $it('fL.originalOrderNo'))[0].item.props.value;
    ...
    setDetailForm(data) {
      ...
      self.reForm.config.forEach(async item => {
        ...
        // 单据日期 退款分类 退款描述 单据来源
        const itemLabelArr = [$it('fL.documentDate'), $it('fL.refundClass'), $it('fL.refundDescription'), $it('fL.sourceDocuments')];
        // const itemLabelArr = ['documentDate', 'refundClass', 'refundDescription', 'sourceDocuments'];
        if (!itemLabelArr.includes(itemLabel)) {
          item.item.props.value = data[dataConfig[itemLabel]];
          // 退款分类
        } else if (itemLabel === $it('fL.refundClass')) {
    ...
    async getDownUp() {
      ...
      if (res.data.code == 0) {
        ...
        self.reForm.config.forEach(item => {
          ...
          // 判责方
          if (item.item.label == $it('fL.responsibleParty')) {
  ...
  computed: {
    // 同步计算申请退款金额
    applyAmt() {
      // return this.reForm.config.filter(item => item.item.label == $it('fL.refundAmount')).item.props.value;
      return this.reForm.config.filter(item => item.item.label == $it('fL.refundAmount'));
    }
  },
  watch: {
    // 监听申请退款金额进行卖家备注赋值
