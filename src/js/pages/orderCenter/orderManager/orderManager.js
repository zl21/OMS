    ...
    return {
      ...
      agTableConfig: {
        ...
        renderArr: {
          ...
          PAY_TIME: params => {
            // 付款时间
            const temp = params.data.PAY_TIME ? this.standardTimeConversiondateToStr(params.data.PAY_TIME) : '';
            return temp;
          },
          AUDIT_TIME: params => {
            // 审核时间
            const temp = params.data.AUDIT_TIME ? this.standardTimeConversiondateToStr(params.data.AUDIT_TIME) : '';
            return temp;
          },
          ...
          CREATIONDATE: params => {
            // 创建时间
            const temp = params.data.CREATIONDATE ? this.standardTimeConversiondateToStr(params.data.CREATIONDATE) : '';
            return temp;
          },

          HOLD_RELEASE_TIME: params => {
            // HOLD单释放时间
            const temp = params.data.HOLD_RELEASE_TIME ? this.standardTimeConversiondateToStr(params.data.HOLD_RELEASE_TIME) : '';
            return temp;
          },

          SCAN_TIME: params => {
            // 出库时间
            const temp = params.data.SCAN_TIME ? this.standardTimeConversiondateToStr(params.data.SCAN_TIME) : '';
            return temp;
          },
          ORDER_FLAG: false
        },
