  ...
  export default {
    ...
    methods: {
      ...
      async getSelectData() {
        const self = this;
        const platformType = await this.getColOption(
          'SG_B_CHANNEL_SYNSTOCK_Q',
          '基本信息',
          'CP_C_PLATFORM_ID'
        );
        const isErrorType = await this.getColOption(
          'SG_B_CHANNEL_SYNSTOCK_Q',
          '基本信息',
          'IS_ERROR'
        );
        const synStatusType = await this.getColOption(
          'SG_B_CHANNEL_SYNSTOCK_Q',
          '基本信息',
          'SYN_STATUS'
        );
        ...
        this.service.inventoryCenter.getChannelSynstockQuery(fromdata)
          ...
          .then(res => {
            ...
            if (res.data.code === 0 && res.data.data.sgChannelStorageStockQResultList.length) {
              res.data.data.sgChannelStorageStockQResultList.map(item => {
