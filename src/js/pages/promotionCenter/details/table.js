      ...
      self.tColumns.forEach((column) => {
        ...
        if (key === 'NUM' || key === 'SUM') {
          column.render = (h, params) => h('input', {
            ...
            on: {
              blur: (event) => {
                params.row[key] = event.target.value == '' || isNaN(event.target.value)
                  ? ''
        ...
        if (key === 'ORDER') {
          column.render = (h, params) => h('input', {
            ...
            on: {
              ...
              keyup: event => event.target.value = event.target.value.match(/^[1-9]\d*$/) ? event.target.value.match(/^[1-9]\d*$/)[0] : ''
            }
          });
        }
      });
