...
export default {
  ...
  data() {
    return {
      ...
      agTableConfig: {
        ...
        agLoading: false,
        columnDefs: [
          {field: "sgCSharePoolEname", headerName: "共享池"},
  ...
  computed: {
    ...
    //         width: '6',
    //         itemdata: {
    //           autoRequest: {precolnameslist: [{iswrite: false, refcol: 'ID', premtype: 'SG_C_SHARE_POOL_ID'}]},
    //           tableRequest: {precolnameslist: [{iswrite: false, refcol: 'ID', premtype: 'SG_C_SHARE_POOL_ID'}]},
    //           col: 1,
    //           colid: 1700826992, // 当前字段的ID
    //           colname: 'SG_C_SHARE_POOL_ID', // 当前字段的名称
    //           datelimit: 'all',
    //           display: 'text', // 显示什么类型，例如xml表示弹窗多选加导入功能，mrp表示下拉多选
    //           fkdisplay: 'mrp', // 外键关联类型
    //           fkdesc: '共享池',
    //           inputname: 'SG_C_SHARE_POOL_ID:ENAME', // 这个是做中文类型的模糊查询字段，例如ENAME
    //           isfk: true, // 是否有fk键
    ...
    //         },
    //         oneObj: e => {
    //           this.formConfig.formValue.SG_C_SHARE_POOL_ID = e.pid;
    //         }
    //       },
    //     ],
    //     formValue: {
    ...
    btnConfig() {
      return {
        ...
        buttons: [
          ...
          // {
          //   type: 'default',
          //   text: window.vmI18n.t('btn.delete'), // 删除 TODO 崔艳吉说先没有删除功能
          //   btnclick: () => {
          //     this.delFun()
          //   }
          // },
  ...
  methods: {
    ...
    getSearchFormData() {
      ...
      formData.append('table', 'V_SG_C_SYNC_GRADIENT_STRATEGY');
    ...
    exportClick(tableName, multipleSelection, objectIds, menu, isExport, type) {
      ...
      formdata.append('menu', '共享池库存梯度策略');
      formdata.append('filename', '共享池库存梯度策略');
    ...
    getTableData() {
      ...
      for (let key in val) {
        if (typeof val[key] === 'boolean') { // 布尔值
          params[key] = val[key]
        } else if (Array.isArray(val[key]) && val[key].length) { // 数组
      ...
      this.service.inventoryCenter.SgSyncGradientStrategyShareQueryList(params).then((res) => {
