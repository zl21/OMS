...
export default {
  ...
  methods: {
    ...
    countChange(event) {
      ...
      if (this.objid == -1) {
        const index = this.inputList.findIndex(n => n.name === window.$it('tL.quantities'));
        if (index != -1) {
    ...
    entry() {
      ...
      if ((this.distribId === '' || this.distribId === undefined)
        ...
        return this.$message({
          message: window.$it('tip.dh'), // '请先选择店仓',
          center: true,
          type: 'warning'
        });
      }
  ...
  mounted() {
    this.inputList.map((obj) => {
      if (obj.name === window.$it('tL.quantities')) {
