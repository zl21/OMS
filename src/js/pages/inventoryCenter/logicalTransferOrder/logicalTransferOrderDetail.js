...
export default {
  props: {
    inputList: {
      ...
      default: () => [
        {
          name: window.vmI18n.t('table_label.quantities'),
          colname: 'QTY',
          defnum: 1
        }
      ]
  ...
  data() {
    return {
      ...
      general: true,
      btnFlag: false,
      isStockBtn: false, // 匹配在库 按钮是否显示
      isStockNoEnoughBtn: false, // 匹配库存不足商品 按钮是否显示
      isUsefulStockBtn: false, // 匹配可用 按钮是否显示
      isDetailMatrix: true,
      tipsDialog: {
        show: false, // 控制警告弹框显示
        title: window.vmI18n.t('modalTitle.warning'), // '警告', // 弹框标题
        type: 'warning', // 类型警告
  ...
  computed: {
    ...
    selectItem() {
      const str = this.routePath.split('/')
  ...
  methods: {
    ...
    objectEdit(e) {
      ...
      this.singleData = e;
      const data = {}
      data[port[this.tablename].tableName] = []
      const formdata = new FormData();
      for (let key in this.amend) {
        const item = this.amend[key]
        data[port[this.tablename].tableName].push({
          ID: item.ID,
      ...
      formdata.append('data', JSON.stringify(e ? e : data))
      ...
      this.service.orderCenter
        ...
        .then(res => {
          ...
          if (data.code === 0) {
            ...
            this.$message({
              message: window.vmI18n.t('modalTips.da'), // '修改折扣成功',
              type: 'success'
            });
            this.discountLoading = false;
    ...
    checkDiscountData() {
      ...
      if (this.updateSalePriceModal.UPDATE_TYPE === 0) {
        if (!this.updateSalePriceModal.SALE_DISCOUNT) {
          warnMsg = window.vmI18n.t('modalTips.db'); // '请维护折扣！';
        }
      } else if (!this.updateSalePriceModal.SALE_PRICE) {
        warnMsg = window.vmI18n.t('modalTips.dc'); // '请维护价格！';
      }
      return warnMsg;
    },
