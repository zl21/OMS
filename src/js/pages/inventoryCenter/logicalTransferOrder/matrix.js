...
export default {
  ...
  props: {
    ...
    stopsave: Boolean,
    distribIds: '',
    isStockBtn: Boolean, //控制匹配在库按钮是否显示
  ...
  data() {
    return {
      ...
      stock: false, // 控制在库
      usable: false, // 控制可用
      chinese: $ChineseDictionary, // 字段集合
  ...
  methods: {
    ...
    async getBodyData() {
      if (this.general) { // 通用矩阵
        if (this.hasStock) {
          let url = this.tablename === 'OC_B_SALE' && this.saleType === 'NOR' ? '/p/cs/oc/v1/sale/queryCommonStorageByPro' : this.stockUrl
          const formdata = new FormData();
          formdata.append('proEcode', this.encode);// 商品编码
          if (this.shippingStock) {
            url = this.shippingStock
          ...
          await axios({
            ...
            let data = res.data;
            if (data.code === 0) {
              this.stockData = data.data; // 发货可用库存
            }
          })
        ...
        if (this.hasReceiving) {
          ...
          let url = this.receivingStock ? this.receivingStock : this.stockUrl
    ...
    findId() {
      if (Object.prototype.toString.call(this.objList) === '[object Array]') {
        ...
        const deliverStockNameArr = ['SENDER_STORE_ID', 'CP_C_STORE_ID', 'CP_C_ORIG_ID']
        const takeStockNameArr = ['CP_C_DEST_ID', 'RECEIVER_STORE_ID']
        const leftDataStoreId = port[this.tablename].leftDataStoreId || '' // 收货店仓 - 取左侧列表里当前行的配销仓
        const leftDataStoreName = port[this.tablename].leftDataStoreName || '' // 收货店仓 - 取左侧列表里当前行的配销仓
          ...
          if (obj.childs) {
            for (const data of obj.childs) {
              ...
              if (takeStockNameArr.indexOf(data.colname) !== -1 || data.name === '收货逻辑仓' || data.colname === port[this.tablename].receivingWarehouse) {
                ...
                this.takeStockName = data.valuedata; // 收货店仓
              } else if (leftDataStoreId) {
                this.takeStock = this.activeObj[leftDataStoreId]; // 收货店仓 id
                this.takeStockName = this.activeObj[leftDataStoreName]; // 收货店仓
              }
              if (data.colname === 'CP_C_PHY_WAREHOUSE_ID' || data.name === '实体仓逻辑') {
                this.cp_c_phy_warehouse_id = data.refobjid || data.pid;
              }
              if (data.colname === 'RETAIL_BEGIN_TIME' || data.name === '零售开始时间') {
                this.retailBeginTime = data.valuedata;
              }
              if (data.colname === 'RETAIL_END_TIME' || data.name === '零售结束时间') {
                this.retailEndTime = data.valuedata;
              }
              if (this.tablename === 'OC_B_SALE' && data.colname === 'SALE_TYPE' || data.name === '销售类型') {
