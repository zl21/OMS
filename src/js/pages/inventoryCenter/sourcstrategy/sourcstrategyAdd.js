  ...
  methods: {
    ...
    sgChannelSourceStrategyQueryFun() {
      ...
      this.service.inventoryCenter.SgChannelSourceStrategyQuery({
        ...
        if (res.data.code === 0) {
          ...
          const beginTime = $utils.dateFormat(new Date(data.beginTime), 'yyyy-MM-dd hh:mm:ss')
          const endTime = $utils.dateFormat(new Date(data.endTime), 'yyyy-MM-dd hh:mm:ss')
    ...
    saveFun() {
      ...
      if (this.ID === '-1' ? formValue1.query_date[0] !== '' : (beginTime !== this.detailData.beginTime || endTime !== this.detailData.endTime)) {
        SG_C_CHANNEL_SOURCE_STRATEGY.BEGIN_TIME = $utils.dateFormat(new Date(formValue1.query_date[0]), 'yyyy-MM-dd hh:mm:ss')
        SG_C_CHANNEL_SOURCE_STRATEGY.END_TIME = $utils.dateFormat(new Date(formValue1.query_date[1]), 'yyyy-MM-dd hh:mm:ss')
      }
