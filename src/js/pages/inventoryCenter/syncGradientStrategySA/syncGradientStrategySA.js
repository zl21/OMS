  ...
  methods: {
      ...
      if (isTrue) {
        ...
        const params = {
          ...
          SG_C_SYNC_GRADIENT_STRATEGY: {
            ...
            // sgCSaStoreId: this.formConfig1.formValue.SG_C_SA_STORE_ID,  //共享池id 共享池梯度策略传，配销仓不传
        ...
        const assembleFun = (item, key, obj) => {
          if (item.colname === key) {
            ...
            if (oldVal !== newVal && newVal) {
              params['SG_C_SYNC_GRADIENT_STRATEGY'][keyObj[key]] = newVal
            }
          }
        }
        ...
        this.service.inventoryCenter.SgSyncGradientStrategySaSave(params).then((res) => {
          if (res.data.code === 0) {
            if (this.ID === '-1') {
      ...
      this.service.common.inputForitem(formdata).then((res) => {
        if (res.data.code === 0) {
          ...
          inpubobj.forEach((item) => {
            ...
            if (item.fkdisplay) {
              const customParams = {precolnameslist: [{iswrite: true, refcol: 'ID', premtype: 'SG_C_SA_STORE_ID'}]}
    ...
    getRightTableDetail() {
      ...
      this.service.inventoryCenter.SgSyncGradientStrategyItemQuery({
        sgCSyncGradientStrategyId: this.ID,
    ...
    onOk() {
      /*this.$comUtils.tabCloseAppoint(this);
       this.$store.commit('customize/TabHref', {
         id: 41460477, // id
         type: 'action', // 类型action
         name: 'SYNCGRADIENTSTRATEGYSALIST', // 文件名
         label: '配销仓库存梯度策略', //  tab中文名
       });
       this.$destroy();*/
      this.$comUtils.tabCloseAppoint(this);
      ...
      this.$store.commit('global/tabOpen', {
        tableId: 249231069,
        type: 'S',
        tableName: 'V_SG_C_SYNC_GRADIENT_STRATEGY_SA',//this.tableName,
        back: true,
      });
    },
    // 返回
