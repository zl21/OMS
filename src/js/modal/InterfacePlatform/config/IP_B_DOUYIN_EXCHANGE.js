...
export default {
  ...
  formConfig: {
    ...
    formData: [
      ...
      {
        style: 'radio', // 输入框类型
        label: window.vmI18n.t('form_label.changeOrderStatus'), // 换货单状态 输入框前文字
        value: 'status', // 输入框的值
        ...
        options: [
          ...
          {
            label: window.vmI18n.t('form_label.exchangePending'), // 换货待处理
            value: '1'
          }
        ]
      },
      {
        style: 'date', // 输入框类型
        label: window.vmI18n.t('form_label.changeOrderModify_time'), // 换货单修改时间 输入框前文字
      ...
      {
        style: 'input', // 输入框类型
        label: window.vmI18n.t('form_label.platformExchangeOrder_no'), // 平台换货单号 输入框前文字
        value: 'bill_no', // 输入框的值
        width: '24', // 所占的宽度 (宽度分为24份,数值代表所占份数的宽度)
  ...
  determine: async (self) => {
    ...
    const param = {
      ...
      table: 'IP_B_DOUYIN_EXCHANGE'
    };
    const fromdata = new FormData();
    fromdata.append('param', JSON.stringify(param));
    // 换货单下载订单
    self.spinShow = true;
    const { data: { code, message } } = await self.service.interfacePlatform.douyinExchangeDownload(fromdata);
    if (code === 0) {
