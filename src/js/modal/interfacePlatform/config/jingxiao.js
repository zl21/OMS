export default {
		...
		formData: [
			{
				...
				inputList: [
					{
						childs: [{ colname: 'CP_C_SHOP_ID', refobjid: 77, valuedata: 77 }]
					}
				],
				itemdata: {
					colid: 167606,
	...
	determine: async (self) => {
		...
		if (downData.formValue.startEndTimes[0] === '' && !downData.formValue.sp_ids && !downData.formValue.orderNum) {
			_this.$Message.warning($it('modalTips.bp')); // 请选择输入日期或输入订单编号
			return;
		}
		const param = {
		...
		const {
			data: { code, message }
		} = await _this.service.common.publicUrlParams('/p/cs/orderDownload', fromdata);
		if (code === 0) {
