  ...
  export default {
    ...
    methods: {
      ...
      saveData() {
        ...
        let url = '';
        if (this.$route.query.source == 2) {
          url = '/p/cs/markSureButton';
        } else if (this.$route.query.source == 3) {
          url = '/p/cs/forcedMatching';
        }
        axios({
          url,
          method: 'post',
        ...
        axios({
          ...
          this.information.formData.forEach((value) => {
            if (value.label === '特殊处理类型') {
              const arr = res.data.data.addcolums[0].childs[12].combobox;
              if (arr && arr.length) {
...
.queryorder .burgeon-modal-content .burgeon-modal-body .burgeon-form-item-content {
