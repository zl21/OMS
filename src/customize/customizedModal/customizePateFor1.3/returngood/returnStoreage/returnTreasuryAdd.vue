    ...
    <div class="queryorderBox">
      ...
      >
        <div class="orderContent">
          <jordanForm :form-config="order.orderform" />
          <jordanButton :btn-config="order.btn" />
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        axios({
          ...
          if (res.data.code === 0) {
            ...
            if (res.data.data.ocBRefundIn.IS_OFF_MATCH == 1) { res.data.data.ocBRefundIn.IS_OFF_MATCH = true; } else if (res.data.data.ocBRefundIn.IS_OFF_MATCH == 0) { res.data.data.ocBRefundIn.IS_OFF_MATCH = false; }
        ...
        axios({
          ...
          this.information.formData.forEach((value) => {
            if (value.label === '特殊处理类型') {
              console.log(res);
              const arr = res.data.data.addcolums[0].childs;
              arr.forEach((item) => {
                if (item.name === '特殊处理类型') {
      ...
      queryBounced() {
        ...
        axios({
          url: '/p/cs/searchButtonsInDetail',
          method: 'post',
          cancelToken: true,
          data: param
        })
