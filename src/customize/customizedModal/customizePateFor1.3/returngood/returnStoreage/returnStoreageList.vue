...
<script>
  ...
  import { isFavoriteMixin } from '../mixins/isFavorite';
  ...
  export default {
          ...
          buttons: [
            ...
            },
            {
              ...
              btnclick: () => {
                ...
                if (ids.length === 0) { return this.$Message.error('请选择一条明细手工匹配'); }
                if (ids.length > 1) { return this.$Message.error('只能选择一条明细手工匹配'); }
                if (this.selection[0].IS_OFF_MATCH == 1) { return this.$Message.error('此退货入库单已经关闭匹配，不允许选择'); }
                const id = ids[0];
                // 需要验证是否能够进入手工匹配界面
                axios({
                  url: '/p/cs/manualMatchingCheck',
                  method: 'post',
                  data: { id }
                }).then((res) => {
                  if (res.data.code === 0) {
                    ...
                    const mes = res.data.message || '状态不匹配不能进入手动匹配';
            ...
            },
            {
              ...
              btnclick: () => {
                const self = this;
                const ids = this.selection.map(item => item.ID);
                if (ids.length === 0) { return this.$Message.error('请选择一条明细错发强制匹配'); }
                if (ids.length > 1) { return this.$Message.error('只能选择一条明细错发强制匹配'); }
                if (this.selection[0].IS_OFF_MATCH == 1) { return this.$Message.error('此退货入库单已经关闭匹配，不允许选择'); }
                const id = ids[0];
                // 需要验证是否能够进入错发强制匹配界面
                axios({
                  ...
                  if (res.data.code === 0) {
                    ...
                    const mes = res.data.message || '状态不匹配不能进入错发强制匹配';
                    self.$Message.error(mes);
                  }
                });
              }
            ...
            { // =======================================================暂时影藏
              ...
              btnclick: () => {
                ...
                axios({
                  url: '/p/cs/returnCancel',
                  method: 'post',
                  data: { ids }
                }).then((res) => {
                  if (res.data.code === 0) {
                    const mes = res.data.message || '作废操作成功';
                    this.$Message.success(mes);
                  } else {
                    const mes = res.data.message || '作废操作失败';
    ...
    methods: {
      ...
      getHeaderList() {
        ...
        const params = {
          table: 'OC_B_REFUND_IN', column_include_uicontroller: true, fixedcolumns: {}, multiple: [], startindex: 0
        };
        axios({
      ...
      requestParams() {
        ...
        if (CREATETIME && CREATETIME !== []) {
          if (CREATETIME[0] !== '') { params.beginDate = CREATETIME[0] ? CREATETIME[0].Format('yyyy-MM-dd hh:mm:ss') : ''; }
          if (CREATETIME[1] !== '') { params.endDate = CREATETIME[1] ? CREATETIME[1].Format('yyyy-MM-dd hh:mm:ss') : ''; }
        }
      ...
      // },
      setTableHeight() {
        const tableHeight = document.getElementsByClassName('jordan-table-box')[0]
          .clientHeight;
        const formHeight = document.getElementsByClassName('searchList')[0].clientHeight;
      ...
      exportClick() {
        ...
        if (self.selection.length) {
          ...
          if (self.tableConfig.data.length === 0) return self.$Message.error('列表没有数据,无法导出!');
