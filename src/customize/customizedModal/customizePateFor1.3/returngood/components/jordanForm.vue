<template>
  <div class="orderManageEdit" :class="formConfig.flodClick?'':'unFlodStyle'" :ref="formConfig.flodClick">
    ...
    <Form class="jordanForm_a" :label-width="100" style="width:100%" :model="formConfig.formValue" :rules="formConfig.ruleValidate" onsubmit="return false;">
      ...
      <Row>
        <Col v-for="(item , index) in formConfig.formData" :key=index :span='item.width'  v-if="item.style !== ''">
          ...
          <FormItem :label="`${item.label}`+ ':'" v-if="item.style === 'date'" :prop="item.value">
            ...
            v-model="formConfig.formValue[`${item.value}`]" :format="item.format" :clearable="item.clearable"
...
<style>
  ...
  /* .ark-form-item-content .ark-icon-md-alarm {
    position: absolute;
    top: 15px;
    right: 5px;
  } */
  ...
  .jordanForm_a .ark-form-item {
    margin-bottom: 0;
  }
  .orderManageEdit .jordanForm_a .ark-input {
    box-sizing: border-box;
    padding: 5px 4px !important;
  }

  .orderManageEdit .jordanForm_a .ark-col-span-6,
  .orderManageEdit .jordanForm_a .ark-col-span-8,
  .orderManageEdit .jordanForm_a .ark-col-span-12,
  .orderManageEdit .jordanForm_a .ark-col-span-24 {
    height: 30px;
    /* margin-top: 8px; */
  }
  /* .orderManageEdit .jordanForm_a .ark-col-span-16 {
    margin-top: 8px;
  } */
  .orderManageEdit .jordanForm_a .ark-input {
    height: 24px;
  ...
  /* 日期组件宽度调整 */
  .orderManageEdit .jordanForm_a .ark-date-picker {
    width: 100%;
  }
  .orderManageEdit .jordanForm_a .ark-input-wrapper {
    line-height: 30px;
  }
  /* 折叠按钮 */
  ...
  .orderManageEdit .popInput .ark-form-item-content {
  ...
  .popSelect .ark-select-single
  .ark-select-selection .ark-select-placeholder,
  ...
  .bothNumInput{
    ...
    /* label样式 */
     .ark-form .ark-form-item-label {
      text-overflow: ellipsis;
      overflow: hidden;
      white-space: nowrap;
      padding: 10px 4px 10px 0 !important;
    }
    .ark-form .ark-form-item-label:hover {
      overflow:inherit;
      white-space:normal;
    }

    .orderManageEdit  .ark-time-picker-cells .ark-time-picker-cells-list{
       box-sizing: border-box;
    }
</style>
