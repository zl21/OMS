<template>
  <div class="jordan-table-box">
    <p class="jordan-table-title" v-if="tableTitle">
    ...
    <!-- 分页 -->
    <div v-if="pageShow" class="jordan-table-pageContent">
      <p style="margin-right:4px;">共{{tableTotal}}条</p>
      <Page
      ...
      <!-- 表格搜索框 -->
      <div class="jordan-search-box" v-if="isShowSearchInput">
        <Input
          v-model="searchValue"
  ...
  methods: {
    ...
    freshTable() {
      ...
      if (this.$route.query.delayTime) {
        let { delayTime } = this.$route.query;
