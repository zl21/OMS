            ...
            },
            {
              ...
              btnclick: (e) => {
                if(this.detailsArrData.length > 1){
                   this.$Message.warning($it('tip.dz'));
                   return false;
                }
                if (
                  this.$route.params.customizedModuleId === "New" &&
  ...
  methods: {
    ...
    totalNum() {
      ...
      self.omsTableConfig.data.forEach((item) => {
        ...
        amt = $utils.accAdd(Number(item[key2]), Number(amt));
        REAL_AMT = $utils.accAdd(Number(item['REAL_AMT']), Number(REAL_AMT));
      });
      setTimeout(() => {
        // 退货明细
        ...
        allDa.forEach((item) => {
          ...
          if (it[pryKey] == item[pryKey]) {
            ...
            if (Number(item.REAL_RETURNABLE_QTY) > sumQ) {
              ...
              const sum = $utils.accAdd(item.REFUND_FEE, it.REFUND_FEE)
    ...
    deleteMainTableData() {
      ...
      if (self.panelReturn) {
        ...
        if (haveGift) {
          ...
          msg = `${$it('tip.jl')}：${haveGift.replace(
            /(，|,)$/,
            " "
          )} ？`;
        } else if (haveGroup) {
          // 请确认是否删除当前选中的退货商品，还存在组合/福袋下挂的其他关联商品
          msg = `${$it('tip.jm')}：${haveGroup.replace(
            /(，|,)$/,
            " "
          )} ？`;
