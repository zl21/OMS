    ...
    returnProduct: {
      async handler(newData) {
        ...
        if (newData === "0") {
          ...
          this.actionTableCon.businessButtonConfig.buttons[1].text = $it('btn.addDetail'); //新增明细
          this.actionTableCon.businessButtonConfig.buttons[2].isShow = true;
    ...
    if (this.$route.params.customizedModuleId !== "New") {
      this.getBtn().then((res) => {
        ...
        if (this.$parent.$parent.panelRef === $it('fL.exchangeDetails')) {
          ...
          // 手工新增
          if (this.$route.query.RETURN_SOURCE !== $it('btn.addManually')) {
  ...
  methods: {
    ...
    totalNum: _.debounce(function () {
      ...
      self.actionTableCon.data.forEach((item) => {
        ...
        realAmt = $utils.accAdd(item.REAL_AMT || 0, realAmt)
      });
      setTimeout(() => {
