<template>
  ...
  >
    ...
    >
      ...
      <span v-if="hasStock" class="ff-matrix--matching">
        <!--数量匹配︰
    ...
    <div v-if="general && hasStock" class="ff-matrix--choose">
      <span v-show="tablename !== 'OC_B_RETURN_ORDER'">
        ...
        >
          <input v-model="usable" type="checkbox" />
        ...
        >
          <input v-model="stock" type="checkbox" />
    ...
    <div v-if="general && hasStock && hasReceiving" class="ff-matrix--choose">
      <span>
        ...
        >
          <input v-model="usableReceiving" type="checkbox" /> 收货可用
        </label>
        <label
          v-if="hasQtyPrein"
          class="ff-matrix--active-six ff-matrix--active-label"
        >
          <input v-model="preinReceiving" type="checkbox" /> 收货在途
        </label>
        <label
          v-if="hasRetail"
          class="ff-matrix--active-five ff-matrix--active-label"
        >
          <input v-model="retailReceiving" type="checkbox" /> 零售数量
        </label>
      </span>
      <span
    ...
    <div v-if="reveal" class="ff-matrix--reveal-choose">
      ...
      >
        <input v-model="pickerR" type="radio" :value="index" />
        {{ list.label }}
      </label>
      <!--<label>
    ...
    <div
      ...
      :class="{
        'ff-matrix--edit-reveal': edit || reveal,
        'ff-matrix--reveal': reveal,
      }"
    >
      <table>
        <thead>
          <tr class="ff-matrix--edit-color">
            ...
            <th v-for="(size, index) of tHead" :key="`'ff-matrix'+${index}`">
              {{ size.name }}
            </th>
          ...
          <tr v-for="(value, key, index) of tBody" :key="index">
            ...
            >
              ...
              <ul v-if="edit || general">
                <li v-if="stock" class="ff-matrix--active-one">
                  {{ value[list.name].stock }}
                </li>
                <li v-if="usable" class="ff-matrix--active-two">
                  {{ value[list.name].usable }}
                </li>
                <!--<li v-if="stockReceiving" class="ff-matrix&#45;&#45;active-three">{{value[list.name].stockReceiving}}</li>-->
                <li v-if="usableReceiving" class="ff-matrix--active-four">
                  {{ value[list.name].usableReceiving }}
                </li>
                <li v-if="preinReceiving" class="ff-matrix--active-six">
                  {{ value[list.name].preinReceiving }}
                </li>
                <li v-if="retailReceiving" class="ff-matrix--active-five">
                ...
                <li v-else class="ff-matrix--table-compile">
                  <input
                    ...
                    :class="{
                      'ff-matrix--input-tip': control[value[list.name].TAG],
                      'ff-matrix--input-disabled': isdisabled || !isActive,
                    }"
                    @focus="inputFocus($event)"
                    @keyup.enter="keyEntry"
                    @input="
                      countChange($event, key, list, value[list.name].TAG)
                    "
...
<script>
import matrix from "@/js/pages/orderCenter/orderManageDetail/orderDetail/matrix.js";

export default matrix;
</script>
<style lang="less" scoped>
@import "~@/css/pages/orderCenter/orderManageDetail/orderDetail/matrix.less";
</style>
