...
<template>
  <div class="batchInfoSet">
    <div class="title">
      <i class="iconfont burgeon-iconliuchengtiaojian"></i>
      <span>条件信息设置</span>
    </div>
    <!--商品来源-->
    <div class="row">
    ...
    <div class="row">
      ...
      <div class="form_content">
        ...
        ></SingleBox>
        <div class="form_item">,最大翻倍数</div>
        <div class="form_el_input form_item doubleNum">
          <input v-model="infoData.max_doubles_limits" oninput = "this.value= this.value.match(/^[1-9]\d{0,2}/) ? this.value.match(/^[1-9]\d{0,2}/)[0] : ''"  placeholder />
        </div>
      </div>
    </div>
    <!--选择商品方式-->
...
export default {
  ...
  computed: {
    ...
    gift_columns() {
      let cols = JSON.parse(JSON.stringify(this.tableCols.giftColumns));
      cols.forEach(column => {
        if (column.key === "ECODE") {
          column.width = 200;
        }
        if (column.key === "SUM_QTY") {
          if ((this.basicData && this.basicData.status === "1") || this.objid == '-1') {
  ...
  watch:{
    'infoData.products_origin': {
      handler(val,old){
        let self = this;
        let cols = JSON.parse(JSON.stringify(this.tableCols.productsColumns));
        let colsxt = JSON.parse(JSON.stringify(this.tableCols.productsColumnsxt));
        let colspt = JSON.parse(JSON.stringify(this.tableCols.productsColumnspt));
        if(val === "1" || val === "2"){
          cols.forEach(column => {
            if (column.key === "ECODE") {
              column.width = 200;
