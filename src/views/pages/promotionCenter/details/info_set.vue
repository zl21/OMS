...
<template>
  <div class="infoSet">
    <div class="title">
      <i class="iconfont iconliuchengtiaojian"></i>
      <span>条件信息设置</span>
    </div>
    <div  v-if="!onlyShowRules">
      ...
      <div class="row"  v-if="showPdtsArr">
        ...
        <div class="form_content">
          <detailtabs
            ...
            :columns="columns"
            :productsArrs="infoData.productsArrs"
            :productsArrsView="productsArrsView"
            :itemdata="itemdata"
      ...
      <div class="row"  v-if="!showPdtsArr">
        ...
        <div class="form_content">
          <detailtable
            :tColumns="columns"
            :tData="productslistView.data"
            :total="productslistView.total"
            :moduleMode="moduleMode"
            :itemdata="itemdata"
            :current="productslistView.current"
            :pageSize="productslistView.pageSize"
          ...
          >
            <div slot="action" class="form_button">
              <ButtonFkDialog :itemdata="itemdataFk"  @getFkChooseItem="getButtonFkChoose"></ButtonFkDialog>
    ...
    <div class="row" v-if="showRulesContent">
        ...
        <div class="form_content">
        <CTSIT v-for="(rule,_index) in infoData.rules" :key="_index" :rule="rule"  :onlyShowRules="onlyShowRules" v-show="showRules(_index,rule)"></CTSIT>
      </div>
    </div>
    <!--导入组件-->
...
export default {
  ...
  computed: {
    itemdata(){
      let self =this;
       ...
       if(this.infoData.products_origin === "2"){
          itemdata.objList = [];
          let obj = this.basicData.stores.itemdata;
    ...
    productsJoin(){
      ...
      if(this.basicData.gradient_gift === "1"){  //梯度赠送选择是
         options = this.groups.productsJoin.filter((item)=>{
            return item.title === "非搭配商品";  //非搭配商品
         });
    ...
    showPdtsArr(){
      let flag = this.infoData.products_join === "2"
      if(flag){  //重置tab页面为第一页
         this.currentTab = 0;
      }
      return flag;
    ...
    onlyShowRules(){  //只显示满足条件规则
      ...
      if(this.basicData.activity_type === "PA" && this.basicData.gradient_gift === "0"){
        ...
        return true;
      }else if(this.basicData.activity_type === "GA" && this.basicData.gradient_gift === "0"){
         this.infoData.rules.forEach((rule) =>{
           rule.show = true;
           if(rule.name === 'QTTY'){
    ...
    includeOrExclude(){  //选择商品方式，非搭配商品全部展示  搭配商品需要去掉【排除以下商品】
         let  options = this.groups.includeOrExclude||[];
         if(this.infoData.products_join === "2"){
            options = this.groups.includeOrExclude.filter((item)=>{
                return item.title !== "排除以下商品";  //非搭配商品
            });
  ...
  methods: {
    ...
    deleteOneTableRowData(tabindex, row ,currentPage ,pageSize){
      ...
      try {
        ...
        let arrs = this.infoData.productsArrs[tabindex].productslist || [];
     ...
     */
    countTablelistView(){
      let rows = this.infoData.productslist ||[];
    ...
    countOneTablelistView(tabindex){
      let rows = this.infoData.productsArrs[tabindex].productslist ||[];
