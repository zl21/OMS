    ...
    computed: {
      ...
      productsJoin() {
        ...
        if (this.basicData.gradient_gift === '1') { // 梯度赠送选择是
          options = this.groups.productsJoin.filter(item => item.title === '非搭配商品' // 非搭配商品
          );
      ...
      includeOrExclude() { // 选择商品方式，非搭配商品全部展示  搭配商品需要去掉【排除以下商品】
        ...
        if (this.infoData.products_join === '2') {
          options = this.groups.includeOrExclude.filter(item => item.title !== '排除以下商品' // 非搭配商品
          );
