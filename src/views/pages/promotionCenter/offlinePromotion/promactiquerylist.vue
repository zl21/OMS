  ...
  <div v-loading="loadings" class="promactiquery">
    <div style="height: 140px;">
      <div class="head_botton">
        ...
        <button v-if="buttons.indexOf(3195)>=0" class="white" @click="promotionClick">新增促销活动</button>
        <button v-if="buttons.indexOf(3196)>=0" @click="Dialog">批量活动下线</button>
        <button v-if="buttons.indexOf(3197)>=0" @click="copy">复制</button> -->

        <button class="active" @click="Demand">查找</button>
        <button class="white" @click="promotionClick">新增促销活动</button>
        <button @click="Dialog">批量活动下线</button>
        <button @click="copy">复制</button>
        <button @click="calcelIssueDialog">取消发布</button>
      ...
      <div class="body_select_three">
        ...
        <div class="item_three_2 filter">

          <myInput :is-active="true" :is-disabled="false" :itemdata="my_input_st.itemdata" />
        </div>
        <!-- <div class="item_three_1">
          <myInputLd :is-active="true" :is-disabled="false" :itemdata="my_input_sh.itemdata" />
        </div> -->
        <!-- 商品 -->
        <div class="item_three_1">
          <span>
...
export default {
  ...
  methods: {
    ...
    errorDialogClose(value, option) {
      if (option) {
        // if(this.dataError.issue ==='issueDialog'){
        //   this.issueAffirm()
        // }
        if(this.dataError.issue ==='batchDown'){
          this.downLine();
        }else if(this.dataError.issue ==='calcelIssueDialog'){
          this.calcelIssue();
        }
      } else {
    ...
    calcelIssueDialog(){//取消发布
      let newList = [],
        flag = false,
        newIds = [];
      // this.$refs["agGridChild" + this.activeName]
      ...
      if (newList.length < 1) {
       ...
       newList.map(item => {
        item.STATUS == 3 ? (flag = 3) : "";
        item.STATUS == 1 ? (flag = 1) : "";
      });
      if (flag === 3) {
        this.$message({
          message: "已下线的促销活动不能取消发布",
          type: "warning"
        });
        return false;
      }
      if (flag === 1) {
        this.$message({
          message: "请选择已发布状态的促销活动取消发布",
          type: "warning"
        });
        return false;
      }
      this.dataError.errorList[0].message = '确定执行取消发布操作？'
      this.dataError.issue = 'calcelIssueDialog';
    ...
    refresh() {
      ...
      if (flag === 1) {
        this.$message({
          message: "请选择已发布状态的促销活动刷新",
          type: "warning"
        });
        return false;
      }
      this.axios({
        url: "/p/cs/pushEsAndRedisByManual",
        method: "POST",
        data: {
          param: JSON.stringify({ids: newIds})
        }
      }).then(res => {
        if (res.data.code === 0) {
          console.log("0", res.data.code);
    ...
    Dialog() {
      ...
      this.dataError.errorList[0].message = '确定执行下线操作？'
      this.dataError.issue = 'batchDown';
      this.dataError.show = true;
    },
    copy() {
     let checkedArr = []
     this.checkedArr = checkedArr.concat(this.checkedArr1).concat(this.checkedArr2).concat(this.checkedArr3).concat(this.checkedArr4)
      ...
      if (this.checkedArr.length > 1) {
       ...
       let _this = {}
        ...
        if(this.checkedArr1.length>0){
         _this = this.tableActiveName1.tBody[this.checkedArr1[0]];
        }else if (this.checkedArr2.length>0) {
            _this = this.tableActiveName2.tBody[this.checkedArr2[0]];
        }else if (this.checkedArr3.length>0) {
            _this = this.tableActiveName3.tBody[this.checkedArr3[0]];
        }else if (this.checkedArr4.length>0) {
            _this = this.tableActiveName4.tBody[this.checkedArr4[0]];
        }

  
        let ACTI_ID = _this.ACTI_ID;
        let typeId = _this.PROM_TYPE_ID;
        ...
        this.axios({
          ...
          if (res.data.code === 0) {
            ...
            //存储种类id保存草稿时需要
            this.$store.state.customize.sch_type_id =
              res.data.data.basic_info.sch_type_id;
...
html .el-pagination .el-pager li, body .el-pagination .el-pager li {
  border: none;
  min-width: auto;
}
.promactiquery {
  ...
  .body_select_three {
    ...
    .item_three_2.filter {
      ...
      .el-autocomplete.table-input.add-input.CP_C_STORE_ID {
        width: 100%;
      }
    }
    .item_three_1 {
