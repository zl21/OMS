  ...
  <div class="procedureTwo_index">
    <div class="procedureTwo_centen">
      ...
      <div class="proc_c_right CommonStyle" v-if="!isOnLine()">
        <h1>活动范围</h1>
        <ul class="M_ul">
          <li class="M_se_input input_m_se">
            <!-- 参与店铺 -->
            <my-input
              :isActive="true"
              :isdisabled="false"
              :itemdata="Mitemdata"
              @getFkChooseItem="handleChangeds1"
            >
            </my-input>
            <!-- 参与仓库 -->
            <my-input
              ...
              :isdisabled="false"
              :itemdata="Mitemdata1"
              @getFkChooseItem="handleChangeds2"
            >
            </my-input>
          </li>
          <!-- <li class="M_se_input">
          ...
          <li class="M_se_input">
            <div class="item_four">
              <my-input
                ...
                :isdisabled="false"
                :itemdata="Mitemdata3"
                @getFkChooseItem="handleChangeds3"
              >
              </my-input>
            </div>
          </li>
      ...
      <div class="FullReduction_1">
        <div class="tableList">
          <div class="condition">
            <table>
              ...
              <td v-show="isShow3()">
                条件商品来源:
                <el-select
                  v-model="combInValue.value6"
                  @change="con_commodity"
                  placeholder="请选择"
                >
                  <el-option
                    v-for="item in combIn.options3"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  >
              ...
              <td v-show="isShow4()">
                优惠商品来源:
                <el-select
                  v-model="combInValue.value7"
                  @change="sale_commodity"
                  placeholder="请选择"
                >
                  <el-option
                    v-for="item in combIn.options4"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  >
              ...
              <td v-show="isShow5()">
                赠品:
                <el-select
                  v-model="combInValue.value8"
                  @change="gift_type_select"
                  placeholder="请选择"
                >
                  <el-option
                    v-for="item in combIn.options5"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  >
              ...
              <td v-show="isShow6()">
                免单商品是否分摊:
                <el-select
                  v-model="combInValue.value9"
                  placeholder="请选择"
                >
                  <el-option
                    v-for="item in combIn.options6"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  >
              ...
              <td v-show="isShow6()">
                优惠基础:
                <el-select
                  v-model="combInValue.value10"
                  placeholder="请选择"
                >
                  <el-option
                    v-for="item in combIn.options7"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  >
              ...
              <td v-show="isShow9()">换购:
                <el-select v-model="combInValue.value12" @change="trade_in" placeholder="请选择">
                  <el-option
                    v-for="item in combIn.options8"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                    >
              ...
              <td>
                是否与优惠券同享:
                <el-select v-model="combInValue.value13" placeholder="请选择">
                  <el-option
                    v-for="item in combIn.options1"
                    :key="item.value"
                    :label="item.label"
          ...
          <div class="double" v-if="showDouble()">
            ...
            <input
              type="text"
              v-model="combInValue.value5"
              @input="handleInput"
            />
            0或者空表示不自动翻倍；-1表示不限次数；正整数表示最大翻倍次数
          </div>
        </div>
      </div>
    </div>
    <div class="remark_box" :class="{ activeRemark: isRemark }">
      <h1 class="M_title" :class="{ active: !isRemark }" @click="clickRemark">
        备注说明
      </h1>
      <div class="remark" v-show="isRemark">
        <el-input
          type="textarea"
          v-model="remark"
          ref="remark"
  ...
  export default {
    data() {
      return {
        ...
        // choose_commodity:{con_commodity_source:1,sale_commodity_source:1}
      }
    },
    computed:{
      list (){
      ...
      schTypeId(){  //中类ID
        return this.$store.state.customize.sch_type_id;
      },
      scheme_data(){
          let view = this.$route.query.view || false;
          if(view){
      ...
      scheme_dataInit(){
        let view = this.$route.query.view || false;
         return  view ? this.schemeOne : this.$store.state.customize.scheme_dataInit
      },
    },
    components: {
      'v-select': select,
    ...
    watch:{
      ...
      // 'combInValue.value6':{
      //   handler(val,old){
      //     this.choose_commodity.con_commodity_source = val;
      //   }
      // },
      // 'combInValue.value7':{
      //   handler(val,old){
      //     this.choose_commodity.sale_commodity_source = val;
      //   }
      // },
    },
    ...
    mounted(){
      ...
      this.orderStatus();
      let routerid = this.$route.query.id;
      this.tyep_current = routerid>0 ?this.scheme_data.detail_id -1 : 0
       this.reloadToData();
    },
    methods:{
      clickRemark(){
      ...
      //执行一下逻辑
      checkSchemeData(){//校验条件商品来源和优惠商品来源
        let routerid = this.$route.query.id;
        let temp =routerid>0 ?this.$store.state.customize.scheme_data :this.$store.state.customize.scheme_dataNew;

        if(temp.promotionType_code==='GA17'||
        temp.promotionType_code==='GA18'||
        temp.promotionType_code==='PA16'||
        temp.promotionType_code==='PA17'){
          this.combInValue.value7 = 2;
        }else{
          if(temp.rules && temp.rules.length>0){
           temp.rules.forEach((item,index)=>{
             if(item.proType){
               this.combInValue.value6 = Number(item.proType)
             }
             if(item.discounts && item.discounts.length>0){
               item.discounts.forEach((i,inx)=>{
                 if(i.proType){
                   this.combInValue.value7 = Number(i.proType)
                 }
               })
             }
           })
         }
         if(temp.limits && temp.limits.length>0){
           temp.limits.forEach((item,index)=>{
             if(item.proType){
               this.combInValue.value7 = Number(item.proType)
             }
           })
         }
        }
       ...
       con_commodity(val){
        this.combInValue.value6 = val;
        this.clear_commodity()
       },
      sale_commodity(val){
         this.combInValue.value7 = val;
          if(this.scheme_data.rules.length>0)
          this.scheme_data.rules.forEach((item,index)=>{
            item.discounts.forEach((f,i)=>{
              f.prizeItem = ''
            })
          })
          this.clear_commodity()
      ...
      clear_commodity(){
         let a = this.$store.state.customize.scheme_dataInit;
         let type = this.tyep_current;
         let routerid = this.$route.query.id
         this.combineInit= false
         this.$nextTick(() => {
            this.combineInit= true
         });
        if( this.scheme_data&& this.scheme_data.detail_code!='GA1307'){
          for(let [index , k] of Object.entries(JSON.parse(a))){
            this.$set( k,'checked',false)
            if(Number(index)===type){
        ...
        if(this.scheme_data.rules.length>0) {
          this.scheme_data.rules.forEach((item,index)=>{
            item.skuIds="";
            item.conditionName="QTTY";
            item.conditionType="GE";
            item.conditionValue="";
            item.preConditionName="DISCOUNT";
            item.preConditionType="G";
            item.preConditionValue=0;
            item.prizeItem="";
            item.prizeNum=1;
            item.content=1;
          })
        ...
        if( this.scheme_data&& this.scheme_data.detail_code=='GA1307'){
          if(this.scheme_data.rules){
            this.scheme_data.rules[0].skuIds = []  
            this.scheme_data.rules[0].uuid = ''
          } 
        }
      },
      gift_type_select(val){
        this.combInValue.value8 = val;
      },
      trade_in(val){//换购
      this.combInValue.value12 = val;
      },
      ...
      handleChangeds1(value){
        ...
        this.Mitemdata.pid = JSON.stringify(valuePid)
      },
      handleChangeds2(value){
        let valuePid = JSON.parse(value.pid)
        if(valuePid.nameList && valuePid.nameList.length>0){
          valuePid.nameList = []
        }
        if(valuePid.lists && valuePid.lists.result && valuePid.lists.result.length>0){
         ...
         this.Mitemdata1.pid = JSON.stringify(valuePid)
      },
      handleChangeds3(value){
        let valuePid = JSON.parse(value.pid)
        if(valuePid.nameList && valuePid.nameList.length>0){
          valuePid.nameList = []
        }
        ...
        this.Mitemdata3.pid = JSON.stringify(valuePid)
      },
      //判断当前方案为线上或线下促销
      isOnLine(){
        //默认为线下
      ...
      reloadToData() {//双击事件
              ...
              this.axios({
                ...
                if(res.data.code===0){
                  ...
                  this.$store.commit('scheme_dataRecover' , JSON.parse(scheme_dataInit));
                  //存储种类id保存草稿时需要
                   this.$store.commit("customize/fnsch_type_id",res.data.data.basic_info.sch_type_id);
                // this.$store.state.customize.sch_type_id = res.data.data.basic_info.sch_type_id
      ...
      tSave(){
        ...
        if(this.Objid>0){
          let routerid = this.$route.query.id
          let dataTwo=routerid>0?
           this.$store.state.customize.dataTwo
          :this.$store.state.customize.dataTwoNew
          for(let i=0;i<dataTwo.scheme_arr.length;i++){
            ...
            if(dataTwo.scheme_arr[i].ID===dataTwo.scheme_attr.sch_id){
              ...
              if(routerid>0){
                this.$store.commit("customize/setscheme_data", dataTwo.scheme_data);
                // this.$store.state.customize.scheme_data= dataTwo.scheme_data
              }else{
                this.$store.commit("customize/setscheme_dataNew", dataTwo.scheme_data);
          ...
          this.remark = dataTwo.basic_info.remark
          let w=[];
          ...
          this.combInValue.value8 = dataTwo.scheme_attr.gift_type
          this.combInValue.value12 = dataTwo.scheme_attr.gift_type || 1 //商品换购 和全场换购中类下面的所有小类都可以选择全部换和梯度换
          this.combInValue.value9 = dataTwo.scheme_attr.is_share
          this.combInValue.value10 = dataTwo.scheme_attr.target_item
        }else{//执行一下逻辑
      ...
      tSaveNoEdit(){
        if(this.Objid>0){
          ...
          this.combInValue.value5 = dataTwo.scheme_attr.max_times
          this.combInValue.value8 = dataTwo.scheme_attr.gift_type
          this.combInValue.value12 = dataTwo.scheme_attr.gift_type
        }
      },
      //是否显示折上折或者商品活动
      isShow1(){
        ...
        try{
           sw = routerid>0?
            this.$store.state.customize.dataTwo.scheme_arr[0].scheme_struct.program_code
            : this.$store.state.customize.dataTwoNew.scheme_arr[0].scheme_struct.program_code
        }catch(e){}
        let t = true
        if(sw === "PA"){
          t = false
      ...
      isShow2(){
        ...
        try{
           sw = routerid>0?
            this.$store.state.customize.dataTwo.scheme_arr[0].scheme_struct
           : this.$store.state.customize.dataTwoNew.scheme_arr[0].scheme_struct
        }catch(e){}
        let t = false
      ...
      isShow3(){
        let t = false;
        let routerid = this.$route.query.id;
        let scheme_data = routerid>0?
         this.$store.state.customize.scheme_data: 
         this.$store.state.customize.scheme_dataNew;
        if(scheme_data.promotionType_code=='GA11'|| 
        scheme_data.promotionType_code=='GA12' || 
        scheme_data.promotionType_code=='GA13' || 
        scheme_data.promotionType_code=='GA14' || 
        scheme_data.promotionType_code=='GA15' || 
        scheme_data.promotionType_code=='GA16' 
        ){
          t=true
        }
        return t
      ...
      isShow4(){
        ...
        scheme_data.promotionType_code=='PA14' ){
          t=true
        }
        return t
      },
      isShow5(){
        let t = false;
        if(this.scheme_data.detail_code==='GA1402'||
      ...
      isShow6(){
        let routerid = this.$route.query.id;
         ...
         :  this.$store.state.customize.scheme_dataNew;
         let t = false;
        if(scheme_data.promotionType_code==='GA16'||
        scheme_data.promotionType_code==='PA15' ||
        scheme_data.promotionType_code === 'GA14' ||
        scheme_data.promotionType_code === 'PA13'){ // 统一送赠品:针对某一类商品，满X元送A商品
          t = true;
        }
        return t;
      },
      isShow9(){
         let routerid = this.$route.query.id;
         let scheme_data = routerid>0?this.$store.state.customize.scheme_data : this.$store.state.customize.scheme_dataNew;
         let t = false;
        if(
        scheme_data.promotionType_code==='GA15'||
        scheme_data.promotionType_code==='PA14'
      ...
      allDel(){        
        ...
        cv.value8 = 1
        cv.value12 = 1
        // this.choose_commodity.con_commodity_source = 1;
        // this.choose_commodity.sale_commodity_source = 1;
        sd.content=1
        let a = JSON.stringify(this.$store.state.customize.scheme_dataInit);
        ...
        for(let [index , k] of Object.entries( this.$store.state.customize.scheme_dataInit)){
          this.$set( k,'checked',false)
          if(Number(index)===val){
            this.$set(k , "checked" , true)
          }
      ...
      showDouble(){
        let routerid = this.$route.query.id
        let t = routerid>0?
        this.$store.state.customize.scheme_data.promotionType_code
         : this.$store.state.customize.scheme_dataNew.promotionType_code
        let sw = true
        switch(t){
          case "GA11":
          case "PA11":
      ...
      switchDetail(val){
        ...
        let routerid = this.$route.query.id
        let scheme_arr = routerid>0?
         this.$store.state.customize.dataTwo.scheme_arr:
         this.$store.state.customize.dataTwoNew.scheme_arr;
       ...
       if(this.sub===scheme_arr[val].ID) return
        ...
        for(let [index , k] of Object.entries(scheme_arr)){
          this.$set( k,'checked',false)
          this.$store.state.customize.scheme_dataInit[index].checked = false;
          if(Number(index)===val){
            this.$set(k , "checked" , true)
            this.$store.state.customize.scheme_dataInit[index].checked = false;
            this.amend=true;
      ...
      save(){     
        ...
        // 商品打折非空判断 除组合打折外
          ...
          if(rule.rules[0].skuIds !== '' && rule.rules[0].conditionValue>=0) {
            if (rule.rules[0].discounts[0].prizeItem==="") {
              this.$message({
                message:"请完善商品条件信息",
                type: "warning"
              })
        ...
        if(rule.promotionType_code == 'GA11' ||
           rule.promotionType_code == 'GA12' ||
           rule.promotionType_code == 'GA13' ||
           rule.promotionType_code == 'GA14' ||
           rule.promotionType_code == 'GA15' ||
           rule.promotionType_code == 'GA16' ||
           rule.promotionType_code == 'PA11' ||
           rule.promotionType_code == 'PA12' ||
           rule.promotionType_code == 'PA13' ||
           rule.promotionType_code == 'PA14' ||
           rule.promotionType_code == 'PA15' 
        ){
          rule.rules.forEach((item,index)=>{
            item.proType= this.combInValue.value6
             if(item.discounts && item.discounts.length>0)
              item.discounts.forEach((i,inx)=>{
                i.proType= this.combInValue.value7
              })
          })
          if(rule.limits && rule.limits.length>0)
          rule.limits.forEach((item,index)=>{
            item.proType= this.combInValue.value7
          })
        }
        let obj={
          ...
          "fixcolumn":{
            ...
            "scheme_attr":{
              ...
              'gift_type':(rule.promotionType_code ==='GA15'||rule.promotionType_code ==='PA14')?this.combInValue.value12:this.combInValue.value8,   //赠品
              'is_share':this.combInValue.value9,   //免单商品是否分摊
              'target_item':this.combInValue.value10,   //优惠基础
              'is_share_with_coupon': this.combInValue.value13, // 是否与优惠券同享
            },
            //5.方案内容（商品、条件、优惠值）
      ...
      console.log("req___:",obj);
        ...
        if(rule.detail_code ==="GA1307"){
          objCopy.fixcolumn.scheme_data.rules[0].skuIds = []
        }
