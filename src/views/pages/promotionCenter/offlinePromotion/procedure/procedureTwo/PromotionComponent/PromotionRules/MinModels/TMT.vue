<template>
  ...
  -->
  <div class="propdis1" >
    <Checkbox v-if="isCheckbox" v-model="rule.mustBuy"></Checkbox>
    <span class="imputStr" :class="{'tmtinput':scheme_data.detail_code === 'GA1106'}" v-if="!label">*</span>
    <my-input
      :isActive="true"
      :isdisabled="false"
...
export default {
  ...
  watch:{
    ...
    'scheme_data.choose_commodity.value6':{
      handler(val,old){
        console.log('scheme_data.choose_commodity.value6',val)
        if(val == 1){
    ...
    handleChanged(value) {
      let detail_code = this.scheme_data ?this.scheme_data.detail_code : "";
       if(value.pid) {
         ...
         console.log(JSON.stringify(valuePid));
  ...
  mounted(){
    ...
    if(this.scheme_data.choose_commodity && this.scheme_data.choose_commodity.value6 == 1){
      this.Mitemdata2.itemdata.reftable = 'PS_C_PRO';
    }else if(this.scheme_data.choose_commodity && this.scheme_data.choose_commodity.value6 == 2){
      this.Mitemdata2.itemdata.reftable = 'PS_C_SKU';
    }
  },
