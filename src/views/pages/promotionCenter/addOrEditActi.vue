...
<script>
  ...
  export default {
    ...
    computed: {
      showInfoDataContainer() { // 全场买赠不展示模块
    ...
    methods: {
      ...
      validate2() {
        ...
        if ((this.basic_info.activity_type === 'PA' || (this.basic_info.activity_type === 'GA' && this.condition_info_setting.products_join === '1')) && this.basic_info.gradient_gift === '0') {
          ...
          if (!o) {
            return { code: -1, message: '【满足条件】至少要选中一项并且填写数量或金额条件' };
          }
        }
        let rs = { code: 0, message: '校验完成' };
        if (this.basic_info.activity_type === 'GA') { // 指定
          if (this.condition_info_setting.products_join === '1') { // 非搭配
            const arrs = this.condition_info_setting.productslist || [];
            if (arrs.length === 0) return { code: -1, message: '无数据' };
            rs = this.checkTable(arrs);
          }
          if (this.condition_info_setting.products_join === '2') { // 搭配
      ...
      validate3() {
        ...
        let rs = { code: 0, message: '校验完成' };
        if (this.basic_info.gradient_gift === '0') { // 不梯度
        ...
        for (const tab of tabs) {
          ...
          if (type === 'info') {
            ...
            for (const rule of rules) {
              if (rule.value === '') {
                return { code: -1, message: `表格【${tab.group}】,数量规则未填写` };
              }
            }
    ...
    mounted() {
      ...
      // 修改草稿
      this.addListener();
      this.$nextTick(() => { this.handleScrollByUser(); });
    },
    beforeDestroy() {
