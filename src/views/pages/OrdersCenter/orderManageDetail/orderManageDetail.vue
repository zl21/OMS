  ...
  export default {
          ...
          buttons: [
            {
              ...
              btnclick: () => {
                ...
                self.$network.post('/api/cs/oc/oms/v1/auditOrder', { ids, type: '1' })
                  .then((res) => {
                    if (res.data.code === 0) {
            ...
            },
            {
              ...
              btnclick: () => {
                ...
                this.$Modal.info({
                  ...
                  cancelText: '确定',
                  onCancel: () => {
                    self.$network.post('/api/cs/oc/oms/v1/auditOrderReserve', { ids, type: '1' })
                      .then((res) => {
                        if (res.data.code === 0) {
            ...
            },
            {
              ...
              btnclick: () => {
                ...
                this.$Modal.info({
                  ...
                  cancelText: '确定',
                  onCancel: () => {
                    self.$network.post('/api/cs/oc/oms/v1/cancelOrder', { ids, type: '1' })
                      .then((res) => {
                        if (res.data.code === 0) {
                          self.$Message.success(res.data.message);
                          self.load();
            ...
            },
            {
              ...
              btnclick: () => {
                ...
                if (self.tab1.order.ID > 0) {
                  const ids = [];
                  ids.push(self.tab1.order.ID);
                  self.$network.post('/api/cs/oc/oms/v1/queryshortagSearchOrder', { ids })
                    .then((res) => {
                      if (res.data.code === 0) {
                        self.$Message.success(res.data.message);
                        self.autoRefresh();
            ...
            },
            {
              ...
              btnclick: () => {
                ...
                if (self.tab1.order.ID > 0) {
                  const ids = [];
                  ids.push(self.tab1.order.ID);
                  self.$network.post('/api/cs/oc/oms/v1/queryFortuneBagShortage', { ids })
                    .then((res) => {
                      if (res.data.code === 0) {
                        self.$Message.success(res.data.message);
                        self.autoRefresh();
            ...
            },
            {
              ...
              btnclick: () => {
                ...
                const ids = [];
                ids.push(id);
                self.$network.post('/api/cs/oc/oms/v1/orderInterception', { ids })
                  .then((res) => {
                    if (res.data.code === 0) {
                      self.$Message.success(res.data.message);
                      self.autoRefresh();
            ...
            },
            {
              ...
              btnclick: () => {
                const self = this;
                const id = self.tab1.order.ID || -1;
                self.$network.post('/api/cs/oc/oms/v1/cancelInterception', { ids: [id] })
                  .then((res) => {
                    if (res.data.code === 0) {
                      self.$Message.success(res.data.message);
                      self.autoRefresh();
            ...
            },
            {
              ...
              btnclick: () => {
                ...
                if (this.$refs.blackListDialog) this.$refs.blackListDialog[0].openConfirm();
              } // 按钮点击事件
            },
            {
              text: '新增工单', // 按钮文本
    ...
    methods: {
      ...
      copyRouteChange(type) {
        ...
        } else if (ORDERSTATUSNAME !== '仓库发货' && ORDERSTATUSNAME !== '平台发货') {
      ...
      load(val) {
        ...
        this.pageLoad = true;
        this.$network.post('/api/cs/oc/oms/v1/getDetail', data)
          .then((res) => {
            this.pageLoad = false;
            if (res.data && res.data.code === 0) {
              const resData = res.data.data;
              resData.TO_SETTLE_STATUS_TAG = data.RESERVE_BIGINT02 === null ? '否' : '是';
              const TO_SETTLE_STATUS_NAME = (this.enumerationList.UPLOAD_SAP_STATUS.find(val => val.value === resData.TO_SETTLE_STATUS) || {}).label;
      ...
      asyncOK() {
        ...
        if (this.error_type === 'auditing') {
          self.$network.post('/api/cs/oc/oms/v1/auditOrder', { ids, type: '2', isCheck: 1 })
            .then((res) => {
              self.$Message.info(res.data.message);
              self.modal = false;
            });
    ...
    created() {
      ...
      this.pageLoad = true;
      this.$network.post(this.$httpApi.public.selectLimitGroups, ['UPLOAD_SAP_STATUS'])
        .then((res) => {
