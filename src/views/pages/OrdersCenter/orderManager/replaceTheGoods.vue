<template>
  <div>
    <div class="order-change-detail">
      <ul>
        <li v-for="(list, index) of queryList" :key="index">
          <label>{{list.label}}</label>
          <Input placeholder v-model.trim="queryParams[list.column]" @on-enter="search"></Input>
        </li>
      </ul>
...
export default {
      ...
      tableConfig: {
        ...
        columns: [
          ...
          },
          {
            ...
            title: "是否组合商品",
            render: (h, params) => {
              let IS_GROUP = params.row.IS_GROUP === "Y" ? "是" : "否";
  ...
  methods: {
    ...
    submit() {
      let self = this;
      let ECODE =
        this.selection && this.selection.ECODE ? this.selection.ECODE : "";
      if (ECODE === "") {
        return self.$Message.error("请选择需要更换的商品");
      }
      if (self.itemskuid === this.selection.skuId)
        return self.$Message.error("不可以选择更换当前商品！");
      let keys = {
    ...
    search() {
      ...
      let isNull = Object.values(p).every(item => {
