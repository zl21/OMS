<template>
  <div class="marking">
    <!-- 会员加急发货 -->
    <div class="vip" v-if="componentData.pageType === 'vip'">
      <span>当前操作会给订单打上加急发货【急】标识，请确认！</span>
    </div>
    <!-- 定金预售提前发货 -->
    <div class="deposit" v-else>
      <re-action-table :jordanTableConfig="depositConfig" />
    </div>
    <jordanBtn style="padding-top: 15px" :btnConfig="btnConfig"></jordanBtn>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    vipConfirmInfo(params) {
      post('/api/cs/oc/oms/v1/orderDeliveryUrgent', params).then(res => {
        if (res.data.code === 0) {
    ...
    getWarehouse() {
      ...
      if (this.depositConfig.jordanFormConfig.formValue.CP_C_SHOP_ID) {
        post(this.$httpApi.public.queryPhyWareHouseList, formData).then(res => {
          if (res.data.code === 0) {
