<template>
  <div class="orderManager-box">
    ...
    <Modal
      v-model="updateSendWmsFlagModal"
      title="标记传仓"
      width="300"
      :mask="true"
      @on-ok="updateSendWms"
      @on-cancel="updateSendWmsCancel"
    >
      <div class="orderContent">
        <jordanForm :form-config="updateSendWmsFormConfig" />
      </div>
    </Modal>
  </div>
</template>
    ...
    methods: {
      ...
      setSearchOption() {
        setTimeout(() => {
          ...
          if (slideBox) {
            setTimeout(() => {
              const pageUl = document.querySelector('.burgeon-dropdown-menu')
                .childNodes[0];
              pageUl.click();
            }, 200);
          }
      ...
      setTableHeight() {
        ...
        const tableHeight = contentHeight - sumHeight;
        self.jordanTableConfig.height = tableHeight - 40;
      },
      ...
      updateSendWms() {
        ...
        axios({
          ...
          method: 'post',
          cancelToken: true,
          data: { ids, MARK_SENDWMS: this.updateSendWmsFormConfig.formValue.MARK_SENDWMS }
        }).then((res) => {
          if (res.data.code === 0) {
            self.$Message.success(res.data.message);
            self.getData();
