              ...
              render: (h, params) => {
                if (
                  ...
                  params.row.QUERYORDERITEMRESULTLIST.forEach((item, index) => {
                    const ecode = item.ecode || '商品';
                    const sizes = item.sizes || '尺寸';
                    const clrs = item.clrs || '颜色';
                    const dataArr = [];
            ...
            },
            {
              ...
              btnclick: () => {
                ...
                if (self.selection.length > 0) {
                  ...
                  if (statusFlag) {
                    self.$Message.warning({
                      content: '选择的订单中存在非待审核订单, 不允许释放库存！',
                      duration: 5,
                      top: 80,
                    });
                    return;
    ...
    methods: {
      ...
      copyRouteChange(type) {
        ...
        if (self.selection.length === 1) {
          ...
          if (type === '原单无效复制') {
            ...
            ORDERSTATUSNAME !== '仓库发货'
            && ORDERSTATUSNAME !== '平台发货'
          ) {
            self.$Message.error(
      ...
      mergeOrder() {
        ...
        for (const item of self.selection) {
          if (
            item.ORDERSTATUSNAME !== '待审核'
            && item.ORDERSTATUSNAME !== '已审核'
          ) {
            message = '要合并的单据的订单状态只能为待审核或已审核';
            break;
          } else if (item.PAYTYPENAME === '货到付款') {
            message = '要合并的单据的付款方式只能为非货到付款';
            break;
          } else if (item.RESERVE_VARCHAR03_NAME !== '非预售') {
            message = '要合并的单据的预售状态只能为非预售';
            break;
          } else if (item.CP_C_PHY_WAREHOUSE_ENAME === '') {
            message = '要合并的单据的发货仓库只能为非空';
            break;
          }
        }
        if (message.length !== 0) {
        ...
        self.$network.post('/api/cs/oc/oms/v1/mergeOrderOne', formdata)
          .then((res) => {
            self.pageLoad = false;
            if (res.data.code === 0) {
              self.$Message.success(res.data.message || '合并订单成功');
              self.getData();
              self.selection = [];
            } else {
              self.$Message.error(res.data.message || '合并订单失败');
            }
          })
          .catch(() => {
            self.pageLoad = false;
      ...
      cancelMergeOrder() {
        ...
        self.selection = self.$refs.agGridChild.AGTABLE.getSelect();
        if (self.selection.length === 0) {
          this.$Message.warning('请选择需要操作的单据！');
          return;
        }
        for (const item of self.selection) {
          if (!['缺货', '待审核', '已审核'].includes(item.ORDERSTATUSNAME)) {
            this.$Message.warning('当前状态异常，不允许操作！');
            return;
          }
        }
        const fromdata = new FormData();
        ...
        self.$network.post('/api/cs/oc/oms/v1/cancelMergeOrder', formdata)
          .then((res) => {
            self.pageLoad = false;
            if (res.data.code === 0) {
              self.$Message.success(res.data.message || '取消合并订单成功');
              self.getData();
              self.selection = [];
            } else {
              self.$Message.error(res.data.message || '取消合并订单失败');
            }
          })
          .catch(() => {
            self.pageLoad = false;
      ...
      checkBatchReturnOrder(selection) {
        ...
        for (let i = 0; i < selection.length; i++) {
          ...
          if (
            ...
            throw new Error(
              `${item.ID}订单需要生成退换货单的订单状态不能已取消、系统作废！`
            );
          } else if (
