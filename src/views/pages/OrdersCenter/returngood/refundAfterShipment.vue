<template>
  <!-- 发货后退款 -->
  <div class="refundAfterShipment" id="cbx">
    <div class="re_button">
      <reButton :btnConfig="btnConfig"></reButton>
    </div>
    <div class="re_form">
      <Collapse v-model="value">
        <Panel name="1">
          基础资料
          <p class="basic-message" slot="content">
            <ImageUpload
            ...
            ></ImageUpload>
            <FormLayout
              :defaultColumn="reForm.defaultColumn"
              :defaultconfig="reForm.config"
              class="basic-message-form"
              ref="FormLayout"
            >
              <div class="cbx" slot="CBX">
                <i style="color: #ff9900;padding: 0 6px;">*</i>
                退款类型:&nbsp;&nbsp;&nbsp;&nbsp;
                <RadioGroup @on-change="billTypeChange" v-model="BILL_TYPE">
                  <Radio label="1">仅退款</Radio>
                  <Radio label="0">退货退款</Radio>
                </RadioGroup>
              </div>
              <div class="returnType" slot="returnType">
                <reForm :formConfig="returnTypeFormConfig"></reForm>
              </div>
              <div class="returnType" slot="returnTypeItem">
                <reForm :formConfig="returnTypeItemConfig"></reForm>
              </div>
            </FormLayout>
          </p>
        </Panel>
        <Panel name="2" v-if="!$route.query.new">
          日志
          <div slot="content">
            <re-form :formConfig="logFormConfig"/>
          </div>
        </Panel>
      </Collapse>
    </div>
    <div class="page-footer">
      <div class="page-footer-navTab">
        <p :class="navStatus === 0 ? 'action' : ''" @click="navStatus = 0">退款单详情</p>
      </div>
      <div class="re_table" v-show="navStatus === 0">
        <reTable
    ...
    <div class="queryorderB">
      ...
      >
        <div class="orderContent">
          <reForm :formConfig="order.orderform"></reForm>
          <reButton :btnConfig="order.btn"></reButton>
        </div>
        <reTable :jordanTableConfig="order.table" @on-current-change="onCurrentChange"></reTable>
      </Modal>
    </div>
    <div class="addItem">
      <Modal
      ...
      >
        <reTable
          :jordanTableConfig="addItem.table"
          @on-select="onSelect"
      ...
      <Modal @on-ok="deleteImgBySure" title="提示" v-model="isModal">
        <p>点击后将删除凭证,是否继续?</p>
      </Modal>
    </div>
  </div>
  ...
  export default {
            ...
            buttons: [
              {
                ...
                btnclick: () => {
                  this.queryBounced();
                  this.$nextTick(() => {
                    let index = this.reForm.config.forEach((val, index) => {
                      if (val.item.label === '单据来源') {
                        this.reForm.config[index].item.props.value = '手动'
                      } else if (val.item.label === '单据日期') {
                        this.reForm.config[index].item.props.value = this.$comUtils.dateFormat(new Date(), 'yyyy-MM-dd')
                      }
                    })
                    ...
                    h("Input", {
                        ...
                        on: {
                          "on-change": e => {
                            ...
                            self.tableConfig.data[params.index] = params.row;
                            let total = 0;
                            self.tableConfig.data.map(item => {
    ...
    methods: {
      ...
      fromOrder(listData, initStatus = false) {
        ...
        let QUERYORDERITEMRESULTLIST = _this.onSelectData.QUERYORDERITEMRESULTLIST;
      ...
      queryorder(listData, isEnter) {
        ...
        if (isEnter) {
          ...
          var QUERYORDERITEMRESULTLIST = _this.onSelectData.QUERYORDERITEMRESULTLIST;
          _this.onSelectData.QUERYORDERITEMRESULTLIST = _this.filter(QUERYORDERITEMRESULTLIST)
      ...
      save() {
        ...
        let AfSend = self.getForm();
        if (AfSend.VIP_PHONE && !/^1[3456789]\d{9}$/.test(AfSend.VIP_PHONE)) return self.$Message.warning('请输入正确的买家手机号')
        AfSend['ID'] = self.$route.query.cid || self.$route.query.id;
        let AfSendItem = self.tableConfig.data.map(item => ({
          id: item.ID,
      ...
      getForm() { //保存获取主表数据
        let self = this;
        let dataConfig = self.reFormDataConfig
        let AfSend = {};
        self.reForm.config.forEach(item => {
          let itemLabel = item.item.label
          AfSend[dataConfig[itemLabel]] = item.item.props.value
          if (itemLabel == '单据来源') AfSend[dataConfig[itemLabel]] = item.item.props.value == '手动' ? 1 : 2;
        })
      ...
      onAddItem() {
        ...
        if (self.$route.query.new || self.$route.query.cid) {
          if (!self.isOne) {
            let data = self.onSelectData;
            //通过原始订单编号二次弹框确定
            self.reForm.config.forEach(item => {
              let configItem = item.item
              let configItemLabel = configItem.label
            ...
            let returnprice = self.getnumPrice(self.tableConfig.data)
      ...
      tableDeleteDetail() {  //删除明细
        ...
        if (self.$route.query.new || self.$route.query.cid || self.$route.query.oid) {  //新增的请求方式
          ...
          let total = self.tableConfig.data.reduce((sum, item) => (sum + number(item.returnPrice || 0)), 0);
          // self.tableConfig.data.forEach(item => {
      ...
      query() {
        let self = this;
        let query = {ID: self.$route.query.cid || self.$route.query.id};
        self.$network.post('/api/cs/oc/oms/v1/copyAfterDeliver', query)
          .then(res => {
            console.log(res);
            if (res.data.code == 0) {
              let resData = res.data.data;
              let AfSend = resData.AfSend;
              ...
              self.logFormConfig.formValue = {
                ...
                CREATIONDATE: AfSend.CREATIONDATE && this.$comUtils.dateFormat(new Date(AfSend.CREATIONDATE), 'yyyy-MM-dd hh:mm:ss'),
                MODIFIERENAME: AfSend.MODIFIERENAME || AfSend.MODIFIERNAME,
                MODIFIEDDATE: AfSend.MODIFIEDDATE && this.$comUtils.dateFormat(new Date(AfSend.MODIFIEDDATE), 'yyyy-MM-dd hh:mm:ss')
              }
              self.$nextTick(() => {
                setTimeout(() => {
                  this.createdStatus = true
      ...
      setDetailForm(data) {
        ...
        self.reForm.config.forEach(async item => {
          ...
          if (itemLabel !== '单据日期' && itemLabel !== '退款分类' && itemLabel !== '退款描述' && itemLabel !== '单据来源') {
            ...
            item.item.props.value = this.$comUtils.dateFormat(new Date(data[dataConfig[itemLabel]]), 'yyyy-MM-dd')
          }
        ...
        this.$network.post('/p/cs/getObject', formdata)
          .then(res => {
            ...
            if (res.data.code == 0) {
              let payType = [];
              let dutyOfficer = [];
              let childs = res.data.data.addcolums[res.data.data.addcolums.length - 2].childs || []
              childs.forEach(item => {
                if (item.name === '判责方') {
      ...
      sellerRemarkValueChange(type = '', value) {
        let config = this.reForm.config
        let index = config.length - 1
        let sellerRemarkData = this.sellerRemarkData
        // 判断编辑状态时是否数据渲染初始化完成
        if (!this.createdStatus) return
        if (type === 'originalOrder') {
          // 原始订单
          for (let key in sellerRemarkData) {
      ...
      // 获取支付方式名称
      payTypeName(value) {
        let item = this.selectOptions['payType'].find(item => item.value === value) || {label: 'xxxxx'}
        return item.label
      },
      ...
      sellerRemarkCompensation() {
        ...
        if (!this.sellerRemarkDataCreated) {
          config.forEach(configItem => {
            let item = configItem.item
            let itemValue = item.props.value || 'xxxxx'
            switch (item.label) {
              case '买家昵称':
    ...
    watch: {
      // 监听申请退款金额进行卖家备注赋值
      applyAmt(newValue) {
        let index = this.reForm.config.length - 1
        let sellerRemarkData = this.sellerRemarkData
