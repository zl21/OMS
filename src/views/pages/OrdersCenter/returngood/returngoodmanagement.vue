  ...
  <div class="returngood">
            ...
            <div class="sales">
              <ul>
                <li>
                  ...
                  <input v-model="amountReturned" type="text" disabled />
                </li>
                <li class="symbol">+</li>
                <li>
                  <!-- 应退邮费 -->
                  <p>{{ vmI18n.t('other.refundablePostage') }}</p>
                  <input v-model="returnPostage" type="text" @input="returnTotal(returnPostage, 1)" />
                </li>
                <li class="symbol">+</li>
                <li>
                  <!-- 其他金额 -->
                  <p>{{ vmI18n.t('other.otherAmounts') }}</p>
                  <input v-model="otherAmount" type="text" @input="returnTotal(otherAmount, 2)" />
                </li>
                <li class="symbol">-</li>
                <li>
                  <!-- 换货金额 -->
                  <p>{{ vmI18n.t('other.exchangeAmounts') }}</p>
                  <input v-model="exchangeAmount" type="text" disabled />
                </li>
                <li class="symbol">=</li>
                <li>
                  <!-- 退货单总金额 -->
                  <p>{{ vmI18n.t('other.totalAmountReturnOrder') }}</p>
                  <input v-model="returnTotalAmount" type="text" disabled @input="isSettlementAmount(returnTotalAmount)" />
                </li>
                <li>
                  <!-- 代销结算金额 -->
                  <p>{{ vmI18n.t('other.settlementAmountConsignment') }}</p>
                  <input v-model="settlementAmount" type="text" @input="isSettlementAmount(settlementAmount)" />
                </li>
              </ul>
            </div>
          </div>
    ...
    <div class="salesTable">
      ...
      <div class="tableBox">
        ...
        />
        <!-- 换货明细 -->
        <business-action-table v-show="labelDefaultValue === 2" :jordan-table-config="jordanTableConfig2" @on-select="returnOnSelect2" @table-delete-detail="returnDeleteDetail2" @on-select-cancel="returnCancel2" @on-select-all="returnSelectAll2" @on-select-all-cancel="returnSelectAllCancel2" />
        <!-- 退货日志 -->
        <OrderItem v-show="labelDefaultValue === 3" :component-data="tab2" />
        <!-- 次品记录 -->
        <business-action-table v-show="labelDefaultValue === 4" :jordan-table-config="jordanTableConfig4" />
      </div>
    </div>
    <div class="queryorderB">
      ...
      <Modal v-model="order.modal" class="queryorder" :mask="true" :title="vmI18n.t('modalTitle.query_OriginalOrderNo')" @on-ok="queryorder" @on-cancel="querycancel">
        ...
        <business-action-table :jordan-table-config="order.table" @on-select="onquerySelect" @on-select-cancel="onqueryCancel" @on-select-all="onSelectAll" @on-select-all-cancel="onSelectAllCancel" />
      </Modal>
    </div>
    <!-- 修改备注 11-->
    <jordanModal
    ...
    <!-- 提示 -->
    <Modal v-model="availableStock" class="available" :title="vmI18n.t('modalTitle.tips')" width="400" :mask-closable="false" :mask="true" @on-ok="saveData" @on-cancel="cancalModal">
      <!-- 。是否继续？ -->
      <p class="availableStock">{{ availableStockMassage }}{{ vmI18n.t('modalTips.n2') }}</p>
    </Modal>
    <Modal v-model="returnDetailAddTable.modal" class="detailAdd" :title="vmI18n.t('modalTitle.newReturnDetails')" @on-ok="resetReturnMainTable" @on-cancel="detailAddCancel">
      <business-action-table :jordan-table-config="returnDetailAddTable.table" @on-select="returnDetailAddOnSelect" @on-select-cancel="returnDetailAddOnCancel" @on-select-all="returnDetailAddOnSelectAll" @on-select-all-cancel="returnDetailAddOnSelectAllCancel" />
    </Modal>
    <!-- 矩阵框-->
    <jordanModal
