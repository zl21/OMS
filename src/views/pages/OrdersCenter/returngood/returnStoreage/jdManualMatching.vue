    ...
    <div class="TreasuryDefault">
      <Collapse v-model="openDefault">
        <Panel name="1">
          基本信息
          <p slot="content">
            <jordanForm @oneObjs="oneObjs" :formConfig="information"></jordanForm>
          </p>
        </Panel>
      </Collapse>
    </div>
...
export default {
  ...
  data() {
    return {
      returnNumber: {
        ...
        url: "returngood/returnStoreage/returnJdNumberPop",
        keepAlive: true,
        ...
        renderArr: [
          {
            ...
            render: (h, params) => {
              ...
              return h("div", [
                ...
                h(
                  "span",
                  `${params.row.OC_B_JD_RETURN_ORDER_ID ? params.row.OC_B_JD_RETURN_ORDER_ID : ''}`
                )
              ]);
            }
          }
  ...
  methods: {
    ...
    saveData() {
      ...
      let url = '';
      if (this.$route.query.source === "2") {
        url = "/api/cs/oc/oms/v1/markSureButtonJd";
      } else if (this.$route.query.source === "3") {
        url = "/api/cs/oc/oms/v1/forcedMatchingJd";
      }
      axios({
        url: url,
        method: "post",
    ...
    queryBounced() {
      ...
      if (lists.id == '' && lists.receive_name == '' && lists.buyer_nick == '' && lists.receive_mobile == '') {
