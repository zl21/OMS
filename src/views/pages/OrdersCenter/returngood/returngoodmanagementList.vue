...
export default {
      ...
      jordanTableConfig: {
        ...
        renderArr: [
                    ...
                    [
                      ...
                      h("i-table", {
                        ...
                        props: {
                          ...
                          "no-data-text": "暂无数据",
                          columns: goodsThead,
                          data: params.row.allSkuItem
                            ? params.row.allSkuItem
                            : [], // 浮框数据
                        },
                      }),
                    ]
          ...
          },
          {
            ...
            render: (h, params) => {
              let imgSrc =
                params.row.RESERVE_VARCHAR02 == null ||
  ...
  methods: {
        ...
        .then((res) => {
          if (res.data.code == 0 && res.data.data.queryResult.length) {
            ...
            for (let i = 0; i < _this.agTableConfig.rowData.length; i++) {
              let item = _this.agTableConfig.rowData[i];
              if (item.MODIFIEDDATE)
                item.MODIFIEDDATE = publicMethodsUtil.DatesTime(
                  item.MODIFIEDDATE
                ); // 修改时间
              if (item.IN_TIME)
                item.IN_TIME = publicMethodsUtil.DatesTime(item.IN_TIME); // 入库时间
              if (item.AUDIT_TIME)
                item.AUDIT_TIME = publicMethodsUtil.DatesTime(item.AUDIT_TIME); // 审核时间
              if (item.LAST_UPDATE_TIME)
                item.LAST_UPDATE_TIME = publicMethodsUtil.DatesTime(
                  item.LAST_UPDATE_TIME
                ); // 退款平台最后修改时间
              if (item.RETURN_CREATE_TIME)
                item.RETURN_CREATE_TIME = publicMethodsUtil.DatesTime(
                  item.RETURN_CREATE_TIME
                ); // 退款创建时间
              if (item.CREATIONDATE)
                item.CREATIONDATE = publicMethodsUtil.DatesTime(
                  item.CREATIONDATE
                ); // 创建时间
              ...
              item.IS_RECEIVE_CONFIRM =
                item.IS_RECEIVE_CONFIRM == 0 ? "否" : "是"; // 是否确认收货
              ...
              _this.$refs.agGridChild.agGridTable(
                _this.agTableConfig.columnDefs,
                _this.agTableConfig.rowData
              );
            }
          } else {
    ...
    getListWork(status = "") {
      ...
      if (_this.$route.query.type == "workID") {
        if (returnParam.time == "IN_TIME")
          _this.formConfig.formValue.IN_TIME = [
            new Date(returnParam.startTime),
            new Date(returnParam.endTime),
          ];
        else if (returnParam.time == "AUDIT_TIME")
          _this.formConfig.formValue.AUDIT_TIME = [
            new Date(returnParam.startTime),
            new Date(returnParam.endTime),
          ];
        else if (returnParam.time == "CREATIONDATE") {
          _this.formConfig.formValue.CREATIONDATE = [
            new Date(returnParam.startTime),
            new Date(returnParam.endTime),
          ];
        ...
        if (returnParam.IS_BACK)
          _this.formConfig.formValue.IS_BACK = returnParam.IS_BACK; // 是否原退
        if (returnParam.CP_C_PHY_WAREHOUSE_ID)
          _this.formConfig.formValue.CP_C_PHY_WAREHOUSE_ID =
            returnParam.CP_C_PHY_WAREHOUSE_ID; // 入库实体仓库
      }
      this.$network
        .axios({
          url: "/api/cs/oc/oms/v1/querySalesReturn",
      ...
      _this.formConfig.formData.forEach((item) => {
        if (item.itemdata) {
          if (item.itemdata.name == "平台")
            ...
            _this.formConfig.formValue.CP_C_PHY_WAREHOUSE_IN_ID =
              item.itemdata.pid;
      ...
      this.$Modal.info({
        ...
        onCancel: () => {
          this.$network
            ...
            .then((res) => {
              if (res.data.code == 0) {
                let mes = res.data.message || _this.vmI18n.t("modalTips.k8");//售后审核成功！
                _this.$Message.success(mes);
                _this.getList(_this.statusTab);
              } else {
                let err = res.data.message ||  _this.vmI18n.t("modalTips.k9");//售后审核失败！
                      ...
                      this.$network
                        ...
                        .then((res) => {
                          ...
                          if (res.data.code === 0) {
                            ...
                            let err = res.data.message || this.vmI18n.t("modalTips.l5");//取消失败！
          ...
          this.$network
            ...
            .then((res) => {
              if (res.data.code == 0) {
                ...
                let err = res.data.message || this.vmI18n.t("modalTips.l5");//虚拟入库失败！
                this.$Message.info(err);
              }
            });
        },
      ...
      this.$network
        ...
        .then((res) => {
          if (res.data.code === 0) {
            ...
            let err = res.data.message || this.vmI18n.t("modalTips.l9");//虚拟仓库入库失败！
            this.$Message.info(err);
          }
        });
    },
      ...
      this.$network
        ...
        .then((res) => {
          if (res.data.code === 0) {
            ...
            let err = res.data.message || this.vmI18n.t("modalTips.m0");//取消自动退款失败！
            this.$Message.info(err);
          }
        });
    },
          ...
          this.$network
            ...
            .then((res) => {
              if (res.data.code === 0) {
                ...
                let err = res.data.message || this.vmI18n.t("modalTips.m4");//批量原退失败！
                this.$Message.info(err);
              }
            });
        },
      ...
      this.$network
        ...
        .then((res) => {
          if (res.data.code === 0) {
            ...
            let err = res.data.message || this.vmI18n.t("modalTips.m6");//从wms撤回失败！
            this.$Message.info(err);
          }
        });
    },
      ...
      this.$network
        ...
        .then((res) => {
          if (res.data.code === 0) {
            ...
            let err = res.data.message || this.vmI18n.t("modalTips.m7");//重WMS失败！
            this.$Message.info(err);
          }
        });
    },
          ...
          this.$network
            ...
            .then((res) => {
              if (res.data.code === 0) {
                ...
                let err = res.data.message || this.vmI18n.t("modalTips.n0");//强制完成失败！
                this.$Message.info(err);
              }
            });
        },
    ...
    cloneRenturnGood() {
      const _this = this;
      if (this.returnSelectData.length !== 1)
        return this.$Message.error(this.vmI18n.t("modalTips.n1"));//请选中一条单据进行复制!
        ...
        this.$network
          ...
          .then((res) => {
            ...
            if (res.data.code == 0 && res.data.data !== null) {
              let mes = res.data.message || _this.vmI18n.t("modalTips.z2");//导出成功！
              _this.$Message.success(mes);
              _this.downloadUrlFile(res.data.data);
              // return (window.location = res.data.data);
            } else {
              let err = res.data.message || _this.vmI18n.t("modalTips.z3");//失败！
              _this.$Message.error(err);
            }
          });
      } else {
        if (_this.agTableConfig.rowData.length === 0)
          return _this.$Message.error(_this.vmI18n.t("modalTips.z4"));//列表没有数据,无法导出!
        if (_this.statusTab == "") {
    ...
    warningOk() {
      ...
      this.$network
        ...
        .then((res) => {
          _this.isExport = false;
          if (res.data.code == 0 && res.data.data !== null) {
            let mes = res.data.message || this.vmI18n.t("modalTips.z2");//导出成功！
            _this.$Message.success(mes);
            // return (window.location = res.data.data);
            _this.downloadUrlFile(res.data.data);
          } else {
            let err = res.data.message || this.vmI18n.t("modalTips.z3");//失败！
            _this.$Message.error(err);
          }
        });
    },
