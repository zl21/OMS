            ...
            formData: [
              {
                ...
                dimChange: (val) => {
                  ...
                  this.$network.post('/p/cs/screenresult', fromdata)
                    .then((res) => {
                      if (res.data.code === 0) {
              ...
              },
              {
                ...
                dimChange: (val) => {
                  ...
                  this.$network.post('/p/cs/skuQuery', {
                    isBlur: 'Y', // N为精确匹配
                    psCSku: {
                      ECODE: val.trim()
                    }
    ...
    methods: {
      ...
      getWarehouse(id, cid) {
        ...
        if (_this.formConfig.formData[0].itemdata.pid) {
          this.$network.post('/p/cs/queryPhyWareHouseList', formData)
            .then((res) => {
              if (res.data.code === 0) {
      ...
      orderCopy() {
        ...
        self.$network.post('/p/cs/billCopy', {
          IDS: [copyID.id],
          TYPE: type
        }).then((res) => {
          if (res.data.code === 0) {
      ...
      getOrderDatail(copyID) {
        const self = this;
        self.$network.post('/p/cs/getOrderDetailList', {
          id: copyID.id,
          currentPage: 1,
          pageSize: 1000
        }).then((res) => {
      ...
      enterQuerySave(value) {
        ...
        if (ecode || value) {
          ...
          self.$network.post('/p/cs/saveBill', data).then((res) => {
            if (res.data.code === 0) {
              if (res.data.data) {
      ...
      querySave(val) {
        const _this = this;
        self.$network.post('/p/cs/skuQuery', {
          isBlur: 'Y', // N为精确匹配
          psCSku: {
            ECODE: val
          }
          ...
          if (res.data.code === 0) {
            ...
            if (_this.jordanTableConfig.data.length) {
              const x = JSON.parse(JSON.stringify(_this.jordanTableConfig.data));
              const d = x.find(
      ...
      tableDeleteDetail() {
        ...
        if (self.ID) {
          self.$network.post('/p/cs/delOrderItem', data).then((res) => {
            if (res.data.code == 0) {
              self.$Message.success(res.data.message);
      ...
      getAddressId(provinceName, cityName, areaName) {
        const self = this;
        return self.$network.post('/p/cs/queryResionByName', {
          provinceName,
          cityName,
          areaName
        });
