<template>
  <div class="order-add">
    <div class="order_add_loading" v-show="isShowFromLoading">
      <Spin></Spin>
    </div>
    ...
    <Collapse v-model="value1">
      <Panel name="1">
        基本信息
        <p slot="content">
          <jordan-form @keyDown="keyDown" :formConfig="formConfig"></jordan-form>
        </p>
      </Panel>
      <Panel name="2">
        收货人信息
        <p slot="content">
          <jordan-form :formConfig="formConfig1"></jordan-form>
        </p>
      </Panel>
      <Panel name="3">
        备注信息
        <p slot="content">
          <jordan-form :formConfig="formConfig2"></jordan-form>
        </p>
      </Panel>
    </Collapse>
    <!-- tab切换 -->
...
export default {
        ...
        buttons: [
          {
            ...
            btnclick: () => {
              ...
              let ocBorderDto = masterTable;
              ocBorderDto.ORDER_STATUS = 1;
              ocBorderDto.ID = self.ID;
              let ocBorderItemDto = self.jordanTableConfig.data;
              //将是否赠品,实物或缺是否转化为YN
              ocBorderItemDto.map(item => {
                if (item.IS_GIFT === "是") {
                  item.IS_GIFT = 1;
                } else if (item.IS_GIFT === "否") {
                  item.IS_GIFT = 0;
                }
              ...
              let data = {
                ocBorderDto: ocBorderDto,
                ocBorderItemDto: ocBorderItemDto
              };
              // 新增订单走新保存（去掉草稿模式）
              if (self.$route.query.id === '-1') return self.saveAdd(data);
              // 如果是丢单复制或者复制订单的保存，传被复制订单的id
              //如果为丢单复制,则新增一个标识
              let copyID = self.$route.query;
              if (copyID.orderCopy) {
                data.type = "2";
        ...
        formData: [
          ...
          },
          {
            ...
            selectChange: () => {
              let _this = this;
              // _this.formConfig.formValue.CP_C_PHY_WAREHOUSE_ENAME
              let optionsArr = _this.formConfig.formData[8].options;
      ...
      jordanTableConfig: {
          ...
          formData: [
            {
              ...
              dimChange: val => {
                ...
                axios({
                  ...
                  if (res.data.code === 0) {
                    let dimList =
                      _this.jordanTableConfig.jordanFormConfig.formData;
        ...
        columns: [
                      ...
                      regx: /^(([1-9]{1}\d*)|(0{1}))(\.\d{0,2})?$/
                                  ...
                                  on: {
                                    "on-change": e => {
                                      ...
                                      if(params.row.QTY) {
                                        params.row.REAL_AMT = publicMethodsUtil.accMul(params.row.QTY, params.row.PRICE_ACTUAL).toFixed(2);
                                      }
          ...
          },
          {
            ...
            render: (h, params) => {
              return h("span", {}, params.row.ADJUST_AMT.toFixed(2));
            }
          },
          {
            title: "优惠金额",
            ...
            render: (h, params) => {
              return h("span", {}, params.row.AMT_DISCOUNT.toFixed(2));
            }
          }
          // {
          //   title: "实物报缺",
  ...
  created() {
    this.$nextTick(() => {
      ...
      this.getDataAccess("OC_B_ORDER", res => {
        // 订单新增-基础信息
        this.formConfig.formData.forEach((parent, parentIndex) => {
        ...
        // 订单新增-收货人信息
        this.formConfig1.formData.forEach((parent, parentIndex) => {
          res.SENSITIVE_COLUMNS.forEach((child, childIndex) => {
            if (parent.dataAcessKey == child.ecode) {
              console.log(parent);
              console.log(child);
        ...
        // 订单新增-备注信息
        this.formConfig2.formData.forEach((parent, parentIndex) => {
          res.SENSITIVE_COLUMNS.forEach((child, childIndex) => {
            if (parent.dataAcessKey == child.ecode) {
              this.setFormPermissions(parent, child, "add");
            }
        ...
        // 订单新增-表格-表单
        this.jordanTableConfig.jordanFormConfig.formData.forEach(
          (parent, parentIndex) => {
            res.SENSITIVE_COLUMNS.forEach((child, childIndex) => {
              if (parent.dataAcessKey == child.ecode) {
                this.setFormPermissions(parent, child, "add");
        ...
        //解决表格表头
        this.jordanTableConfig.columns = this.columns = this.setTablePermissions(
  ...
  methods: {
    ...
    getWarehouse(id, cid) {
      ...
      if (id) {
        formData.append('param', JSON.stringify({"shopId": id, "cityShopFlag": _this.formConfig1.formValue.RESERVE_BIGINT12}));
      } else {
        formData.append('param', JSON.stringify({"shopId": _this.formConfig.formData[0].itemdata.pid}))
      }
      if (_this.formConfig.formData[0].itemdata.pid) {
        axios({
          url: "/p/cs/queryPhyWareHouseList",
          method: "post",
          data: formData
        }).then(res => {
          if (res.data.code === 0) {
    ...
    //判断付款方式值的变化,隐藏显示某些表单
    watch_paytype() {
      let showNo = this.formConfig.formData;
      let selectData = this.formConfig.formValue.PAY_TYPE;
      if (selectData === 1) {
        showNo.map(item => {
    ...
    watchPlatform(platform,expressCode){
      let self =this;
      let formData = self.formConfig.formData;
      if (platform === 50) {
        formData.map(item => {
    ...
    sava(data) {
      let self = this;
      let warnFlag = self.saveWarnCheck(data);
      if(warnFlag){
        self.$Modal.info({
          title: '提示',
          content: '订单存在非赠品的成交金额为0， 是否确定保存订单？',
          mask: true,
          showCancel: true,
          okText: '确定',
          cancelText: '取消',
    ...
    saveWarnCheck(data){
      let items = data.ocBorderItemDto;
      let warnFlag = false;
      ...
      items.map(item=>{
        if(item.IS_GIFT == "0"){
          ...
          realAmtTotal = publicMethodsUtil.accAdd(
            parseFloat(item.REAL_AMT).toFixed(2),realAmtTotal
          );
        }
      });
    ...
    enterQuerySave(value) {
      ...
      let ecode = self.jordanTableConfig.jordanFormConfig.formValue.dimData;
      let Num = self.jordanTableConfig.jordanFormConfig.formValue.Num;
        ...
        axios({
          ...
          if (res.data.code === 0) {
            ...
            axios({
              url: "/p/cs/getOrderDetailList",
              method: "post",
              // data: { ps_c_sku_ecode: ecode , id:self.ID }
              data: {
                id: self.ID,
                currentPage: 1,
                pageSize: 1000
            ...
            self.$Message.warning(res.data.message || "明细保存失败!");
          }
        });
      } else {
        self.$Message.warning("请输入条码!");
      }
    },
    enterQuerySave1(val) {
    ...
    querySave(val) {
      ...
      axios({
        ...
        if (res.data.code === 0) {
          ...
          let cloneArr = [];
          if (!data.length) return _this.$Message.error('不存在该条码！');
          data.forEach(item => {
            cloneArr.push({
          ...
          if (_this.jordanTableConfig.data.length) {
            let x = JSON.parse(JSON.stringify(_this.jordanTableConfig.data));
            let d = x.find(
              item =>
                item.PS_C_SKU_ECODE === cloneArr[0].PS_C_SKU_ECODE
            );
            if (d) {
              ...
              _this.jordanTableConfig.data = _this.jordanTableConfig.data.concat(cloneArr);
            }
          } else {
    ...
    onSelectCancel(selection, row) {
      ...
      self.jordanTableConfig.data.map(item => {
        if(row.ID){
          ...
          if (item.PS_C_SKU_ECODE === row.PS_C_SKU_ECODE) {
            item["checked"] = false;
          }
        }
      }); //表格勾选取消事件
    ...
    setOrderData(data) {
      ...
      self.formConfig.formValue.is_kaip = data.IS_INVOICE === 1 ? true : false; //是否开票
      ...
      self.formConfig1.formData.map(item => {
        ...
        } else if (item.itemdata && item.itemdata.name === "收货人区") {
