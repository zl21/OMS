  ...
  export default {
    ...
    methods: {
       ...
       */
        ...
        axios({
          method: 'post',
          url: '/p/cs/sg/v1/warehouse/sgBInPickorder/querySgBInPickorder',
          data: formdata
        }).then((res) => {
          console.log(res);
          if (res.data.code === 0) {
            res.data.data.sgBInPickorderItemList.map((item) => { // 计算总数
              self.scanOutbound.workRTK.total = self.scanOutbound.workRTK.total + item.QTY_NOTICE;
              self.scanOutbound.workRTK.scanCount = self.scanOutbound.workRTK.scanCount + item.QTY_IN;
            });
            ...
            self.notScanData.data = JSON.parse(JSON.stringify(res.data.data.sgBInPickorderItemList)); // 赋值未扫描商品明细
            self.notScanData.init_data = JSON.parse(JSON.stringify(res.data.data.sgBInPickorderItemList)); // 最初值赋值未扫描商品明细
            self.setData(JSON.parse(JSON.stringify(res.data.data.sgBInPickorder)));
            self.setPickOrder(JSON.parse(JSON.stringify(res.data.data.sgBInPickorderItemList)));
            self.setBinningDetail(JSON.parse(JSON.stringify(res.data.data.sgBInPickorderTeusItemList)));
            self.setSourceNotification(JSON.parse(JSON.stringify(res.data.data.sgBInPickorderNoticeItemList))); // 来源入库通知单
            self.setEntryNotice(JSON.parse(JSON.stringify(res.data.data.sgBInPickorderResultItemList))); // 来源入库结果单
            self.setTeusDetail(JSON.parse(JSON.stringify(res.data.data.sgBInPickorderTeusItemList)));
          } else {
      ...
      existTues(data, isNull) {
        ...
        if (isNull === 1) { // 从拣货明细中查到的相同条码
          if (currentTues.some(item => (item.PS_C_SKU_ECODE === data.PS_C_SKU_ECODE || item.GBCODE === data.GBCODE))) { // 如果当前箱明细中有相同条码
