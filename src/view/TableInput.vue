...
<template>
  <div class="tableInput">
    ...
    >
      <Row>
        <Col
          v-for="(item, index) in tableFormConfig.formData"
          :key="index"
          :span="item.width"
        >
          <!-- 日期選擇 -->
          <FormItem v-if="item.style === 'date'" :prop="item.value">
            <DatePicker
              ...
              :transfer="true"
              @on-change="runMethods(item.onChange(item.value))"
            ></DatePicker>
          </FormItem>
          ...
          <FormItem v-if="item.style === 'popInput'" class="popInput">
            <my-input
              ...
              @getFkChooseItem="runMethods(item.tableObj(item.itemdata))"
            ></my-input>
          </FormItem>
          ...
          >
            <Select
              @on-change="runMethods(item.selectChange($event))"
              v-model="item.value"
          ...
          <FormItem v-if="item.style === 'checkbox'" :prop="item.value">
            <Checkbox
              v-model="item.value"
              :type="'checkbox'"
              @on-change="runMethods(item.radioChange(item.value))"
            ></Checkbox>
          </FormItem>
        </Col>
      </Row>
