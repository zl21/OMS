        ...
        >
          <template v-if="item.style !== ''">
            ...
            >
              ...
              <Input
                :autocomplete="
                  item.autocomplete ? item.autocomplete : 'new-password'
                "
                ...
                v-model.trim="
                  formConfig.formValue[`${item.value || item.colname}`]
                "
                ...
                @on-change="
                  runMethods(item.inputChange ? item.inputChange() : '')
                "
              ></Input>
              <!-- 输入框图标 -->
            ...
            >
              <Input
                ...
                :maxlength="item.maxlength"
                :autosize="
                  item.autosize ? item.autosize : { minRows: 2, maxRows: 5 }
                "
                :placeholder="item.placeholder ? item.placeholder : ''"
                @on-enter="runMethods(item.inputenter)"
                @on-blur="runMethods(item.inputblur)"
            ...
            <FormItem
              ...
              :class="
                item.class ? `${item.class}` + ' ' + 'popSelect' : 'popSelect'
              "
              :prop="item.value || item.colname"
            >
              <Select
                @on-change="
                  (val) =>
                    runMethods(item.selectChange ? item.selectChange(val) : '')
                "
                @on-open-change="
                  (val) =>
                    runMethods(item.onOpenChange ? item.onOpenChange(val) : '')
                "
                label-in-value
                v-model="formConfig.formValue[`${item.value || item.colname}`]"
                ...
                @on-clear="
                  runMethods(
                    item.clearSelect
                      ? item.clearSelect(item.value || item.colname)
                      : false,
                    true
                  )
                "
            ...
            >
              <my-input
                ...
                @inputBlur="
                  (row) =>
                    runMethods(
                      typeof item.InputBlur == 'function' &&
                        item.InputBlur(
                ...
                @itemInputEnter="
                  (row) =>
                    runMethods(
                      typeof item.InputEnter == 'function' &&
                        item.InputEnter(
                          item.itemdata.isBackRowItem ? row : item.itemdata
                        )
                    )
            ...
            >
              ...
              >
                <Radio
                  ...
                  :label="option.value"
                  :disabled="option.disabled"
                  >{{ option.label || option.title }}</Radio
                >
              </RadioGroup>
            </FormItem>
            ...
            >
              ...
              <template v-else-if="item.options && !item.onlyBox">
                <Checkbox
                  v-if="item.hasCheckAll"
                  class="checkAllItem"
                  style="float: left"
                  :indeterminate="item.indeterminate"
                  :value="item.checkAll"
                  @click.prevent.native="
                    runMethods(item.handleCheckAll ? item.handleCheckAll : '')
                  "
                  >全选</Checkbox
                >
                <CheckboxGroup
                  ...
                  @on-change="
                    (val) =>
                      runMethods(
                        item.checkboxChange ? item.checkboxChange(val) : ''
                      )
                  "
                >
                  <Checkbox
                    ...
                    :key="index"
                    :disabled="option.disabled"
                    >{{ option.label || option.title }}</Checkbox
                  >
                </CheckboxGroup>
              </template>
            </FormItem>
            ...
            >
              <AttachFilter
                ...
                @on-blur="runMethods(item.dimblur)"
              ></AttachFilter>
            </FormItem>
            <!-- 数字框 -->
            <FormItem
              ...
              <div class="bothNumInput">
                <div class="bothNumLeft">
                  <InputNumber
                    ...
                    :min="item.min"
                    v-model.trim="
                      formConfig.formValue[`${item.value || item.colname}`][0]
                    "
                    :step="item.step"
                    :formatter="item.formatter"
                    :parser="item.parser"
                ...
                <div class="bothNumRight">
                  <InputNumber
                    ...
                    :min="item.min"
                    v-model.trim="
                      formConfig.formValue[`${item.value || item.colname}`][1]
                    "
                    :step="item.step"
                    :formatter="item.formatter"
                    :parser="item.parser"
            ...
            >
              ...
              >
                <span v-if="item.showText" slot="open">{{
                  item.showText.open || "开启"
                }}</span>
                <span v-if="item.showText" slot="close">{{
                  item.showText.open || "关闭"
                }}</span>
              </i-switch>
            </FormItem>
            ...
            >
              <slot
                :rowData="{ item, value: formConfig.formValue }"
                :rowIndex="index"
                :name="item.slotName"
              />
            </FormItem>
          ...
          <Tooltip
            ...
            :placement="item.tips.placement || 'bottom'"
