...
export default {
  ...
  activated() {
    ...
    this.$comUtils.setTableHeight(this, 50);
    // 检测屏幕变化 设置高度 重新渲染agTabe
    this.$comUtils.onresizes(this, 10);
  },
  created() {
    const self = this;
    self.getSelectData();
  ...
  methods: {
    ...
    async getSelectData() {
      ...
      const arrBillType = await this.getColOption(
        ...
        // "基本信息",
        window.vmI18n.t('common.baseInformation'),
        'BILL_TYPE'
      );
    ...
    async unCustAudit() {
      ...
      if (data.code === 0) {
        self.$Message.success(window.vmI18n.t('modalTips.z1')); // 反客审成功!
        self.getList();
      } else {
        // self.$Message.error(data.message);
      }
