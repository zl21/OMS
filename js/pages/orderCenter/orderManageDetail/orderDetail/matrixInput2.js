...
import matrix from 'allpages/orderCenter/orderManageDetail/orderDetail/matrixPop.vue'
import DragDialog from 'framework/components/dialog/dragDialog.vue'

const { store } = R3
const port = require('_/config/config/orderDetailConnector').default
...
export default {
  ...
  methods: {
    ...
    countChange(event) {
      ...
      if (this.objid == -1) {
        const index = this.inputList.findIndex(
          (n) => n.name === window.vmI18n.t('table_label.quantities')
        )
        if (index != -1) {
    ...
    async getStyle() {
      ...
      const {
        data: { code, data },
      } = await this.service.common.publicUrlParamsGet(port.judgePro, {
        params,
      })
      if (code === 0) {
        return data
    ...
    entry() {
      ...
      this.NoImport = true
      if (this.sub !== -1) {
        if (this.sub > this.lists.length - 1) {
          this.search = ''
        } else if (this.lists.length === 0) {
          this.search = this.lists[0].ECODE.toLocaleUpperCase()
        } else {
      ...
      if (
        (this.distribId === '' || this.distribId === undefined) &&
        (this.cp_c_phy_warehouse_id === '' ||
          this.cp_c_phy_warehouse_id === undefined) &&
      ...
      this.getStyle().then((data) => {
        ...
        if (data.TYPE === 'proEcode') {
          ...
          obj[
            ...
            this.count === '' ? this.matrixnum : this.count // 数量
          if (this.objid == -1) {
            if (index !== -1) {
              if (this.inputList[index].matrix !== undefined)
                this.$delete(this.inputList[index], 'matrix')
              ...
              if (QTY !== -1) {
                this.$set(
                  this.inputList[QTY],
                  'valuedata',
                  this.count === '' ? this.matrixnum : this.count
                )
    ...
    findId() {
      ...
      if (
        ...
        Object.keys(data.add).length !== 0
      ) {
        if (data.add[this.tablename].CP_C_STORE_ID)
          id = data.add[this.tablename].CP_C_STORE_ID
        if (!id && data.add[this.tablename].CP_C_DEST_ID)
          id = data.add[this.tablename].CP_C_DEST_ID
        if (!id && data.add[this.tablename].CP_C_ORIG_ID)
          id = data.add[this.tablename].CP_C_ORIG_ID
        if (!id && data.add[this.tablename].CP_C_PHY_WAREHOUSE_ID)
          this.cp_c_phy_warehouse_id =
            data.add[this.tablename].CP_C_PHY_WAREHOUSE_ID
      } else if (Object.keys(data.modify).length !== 0) {
        if (
          Object.keys(data.default).length &&
          data.modify[this.tablename].CP_C_STORE_ID
        )
          id = data.modify[this.tablename].CP_C_STORE_ID
        if (!id && data.modify[this.tablename].CP_C_DEST_ID)
          id = data.modify[this.tablename].CP_C_DEST_ID
        if (!id && data.modify[this.tablename].CP_C_ORIG_ID)
          id = data.modify[this.tablename].CP_C_ORIG_ID
        if (!id == '' && data.modify[this.tablename].CP_C_PHY_WAREHOUSE_ID)
          this.cp_c_phy_warehouse_id =
            data.modify[this.tablename].CP_C_PHY_WAREHOUSE_ID
      }
      if (!id && Object.keys(data.default).length !== 0) {
        if (data.default[this.tablename].CP_C_STORE_ID)
          id = data.default[this.tablename].CP_C_STORE_ID
        if (!id && data.default[this.tablename].CP_C_DEST_ID)
          id = data.default[this.tablename].CP_C_DEST_ID
        if (!id && data.default[this.tablename].CP_C_ORIG_ID)
          id = data.default[this.tablename].CP_C_ORIG_ID
        if (!id && data.default[this.tablename].CP_C_PHY_WAREHOUSE_ID)
          this.cp_c_phy_warehouse_id =
            data.default[this.tablename].CP_C_PHY_WAREHOUSE_ID
      }
    ...
    async save(val) {
      if (val && this.isCustom) {
        ...
        if (Object.keys(this.customData).length !== 0 && !this.judgeSingle) {
          ...
          try {
            // 修改后未验证
            const res = await this.service.common.publicUrlParams(
              port[this.tablename].amendBody,
              params
            )
            const data = res.data
            if (data.code === 0) {
        ...
        if (Object.keys(this.singleData).length !== 0 && this.judgeSingle) {
          ...
          try {
            const res = await this.service.common.publicUrlParams(
              port[this.tablename].singleCode,
              params
            )
