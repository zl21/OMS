...
export default {
  ...
  methods: {
    ...
    async getAllData() {
      ...
      const { data } = await self.service.orderCenter.querySgBOutPickorder(formdata);
      if (data.code === 0) {
        data.data.sgBOutPickorderItemList.forEach(item => {
          // 计算总数
          self.scanOutbound.workRTK.total += item.QTY_NOTICE;
          self.scanOutbound.workRTK.scanCount += item.QTY_OUT;
        });
        ...
        self.notScanData.data = JSON.parse(JSON.stringify(data.data.sgBOutPickorderItemList)); // 赋值未扫描商品明细
        self.notScanData.init_data = JSON.parse(JSON.stringify(data.data.sgBOutPickorderItemList)); // 最初值赋值未扫描商品明细
        self.setData(JSON.parse(JSON.stringify(data.data.sgBOutPickorder)));
        self.setPickOrder(JSON.parse(JSON.stringify(data.data.sgBOutPickorderItemList)));
        self.setBinningDetail(JSON.parse(JSON.stringify(data.data.sgBOutPickorderTeusItemList)));
        self.setSourceNotification(JSON.parse(JSON.stringify(data.data.sgBOutPickorderNoticeItemList))); // 来源出库通知单
        self.setEntryNotice(JSON.parse(JSON.stringify(data.data.sgBOutPickorderResultItemList))); // 来源出库结果单
        self.setTeusDetail(JSON.parse(JSON.stringify(data.data.sgBOutPickorderTeusItemList)));
      } else {
    ...
    existTues(data, isNull) {
      ...
      if (isNull === 1) {
        // 从拣货明细中查到的相同条码
        if (currentTues.some(item => item.PS_C_SKU_ECODE === data.PS_C_SKU_ECODE || (item.GBCODE && item.GBCODE === data.GBCODE))) {
          // 如果当前箱明细中有相同条码或国标码
    ...
    async save() {
      ...
      const { data } = await this.service.orderCenter.saveOutPickOrder(formdata);
      if (data.code === 0) {
    ...
    async audit() {
      ...
      if (self.ID === -1) {
        ...
        param.IS_ALLOW_PART = self.scanOutbound.scanWork.isAllowPart ? 'Y' : 'N';
        formdata.append('param', JSON.stringify(param));
        const { data } = await this.service.orderCenter.auditOutPickOrder(formdata);
        if (data.code === 0) {
          self.$Message.success(data.message);
          // self.reload();
        } else {
      ...
      if (type === '条码') {
        ...
        self.notScanData.data.forEach(item => {
          if (item.PS_C_SKU_ECODE === data) {
            if (self.scanOutbound.scanJob.addOrSub === 'add' && !operationType) {
              // 扫描
              // item.QTY_NOTICE = item.QTY_NOTICE - num
              item.QTY_OUT += num;
              item.QTY_DIFF -= num;
            } else if (self.scanOutbound.scanJob.addOrSub === 'sub' || operationType === 'sub') {
              // 反冲
              // item.QTY_NOTICE = item.QTY_NOTICE + num;
              item.QTY_OUT -= num;
              item.QTY_DIFF += num;
        ...
        self.scanOutbound.delArr.forEach(item => {
          self.scanOutbound.jordanTableConfig.data.forEach((sItem, index) => {
            if (item.ID === sItem.ID) {
              ...
              if (sItem.QTY_TEUS > 0) {
                self.scanOutbound.workRTK.scanCount -= sItem.QTY_TEUS;
                self.scanOutbound.workRTK.unscanCount += sItem.QTY_TEUS;
                self.updataNotScan(sItem.PS_C_SKU_ECODE, '条码', 'sub'); // 更新未扫描商品
                // self.IMAGE = data.IMAGE ? JSON.parse(data.IMAGE)[0].URL : ''; //展示扫描图片
              }
            }
          });
