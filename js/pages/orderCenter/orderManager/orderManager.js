  ...
  export default {
        ...
        extendBtn: [
          ...
          },
          {
            ...
            btnclick:()=>{
              ...
              if(!self.selection.length){
                $utils.msgTips(self, 'warning', '请选择需要标记的单据' , 0);
                return;
              }
    ...
    methods: {
      ...
      urgentShipment() {
        ...
        const IDS = self.selection.map((item) => item.ID);
        if (!IDS.length) {
          $utils.msgTips(self, 'warning', '请选择需要打标的单据!', 0);
          return;
        }
      ...
      returnConfirm(){  //退货确认
        ...
        if(!self.selection.length){
          $utils.msgTips(self, 'warning', 'ju');
          return;
        }
        if(self.selection.length > 1){
          $utils.msgTips(self, 'warning', 'kg');
          return;
        }
