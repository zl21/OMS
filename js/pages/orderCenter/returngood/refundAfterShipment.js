              ...
              btnclick: () => {
                ...
                this.$nextTick(() => {
                  this.reForm.config.forEach((val, index) => {
                    ...
                    if (
                      val.item.label ===
                      window.vmI18n.t('form_label.sourceDocuments')
                    ) {
                      this.reForm.config[index].item.props.value = '手动';
                      // '单据日期'
                    } else if (
                      val.item.label ===
                      window.vmI18n.t('form_label.documentDate')
                    ) {
                      this.reForm.config[
                        index
      ...
      reForm: {
        ...
        config: [
          ...
          },
          {
            item: {
              type: 'Input',
              label: window.vmI18n.t('form_label.payAccount'), // '支付账号',
              required: true,
              props: {
                value: '',
              },
    ...
    if (
      ...
      this.$nextTick(() => {
        this.reForm.config.forEach((val, index) => {
          // '单据来源'
          if (
            val.item.label === window.vmI18n.t('form_label.sourceDocuments')
          ) {
            this.reForm.config[index].item.props.value = '手动';
            // '单据日期'
          } else if (
            val.item.label === window.vmI18n.t('form_label.documentDate')
          ) {
  ...
  methods: {
    ...
    fromOrder(listData, initStatus = false) {
      ...
      _this.reForm.config.forEach((item) => {
        // 原始订单编号
        if (item.item.label == _this.vmI18n.t('form_label.originalOrderNo'))
          item.item.props.value = data.ID;
        // 店铺名称
        if (item.item.label == _this.vmI18n.t('table_label.shopName'))
          item.item.props.value = data.CP_C_SHOP_TITLE;
        // 原始平台单号
        if (item.item.label == _this.vmI18n.t('form_label.originalPlatformNo'))
          item.item.props.value = data.SOURCE_CODE;
        // 买家昵称
        if (item.item.label == _this.vmI18n.t('table_label.buyerNickname'))
          item.item.props.value = data.USER_NICK;
        // 买家手机号;
        if (item.item.label == _this.vmI18n.t('form_label.BuyerPhoneNumber'))
        ...
        // 支付方式
        if (item.item.label == _this.vmI18n.t('table_label.paymentWay'))
        ...
        // 收款人姓名
        if (item.item.label === _this.vmI18n.t('form_label.namePayee'))
        ...
        // 申请退款金额
        if (
          item.item.label == _this.vmI18n.t('form_label.apply_refundAmount')
        ) {
    ...
    getForm() {
      ...
      self.reForm.config.forEach((item) => {
        ...
        if (itemLabel == window.vmI18n.t('form_label.sourceDocuments')) {
          // '手动'
          AfSend[dataConfig[itemLabel]] =
            item.item.props.value == window.vmI18n.t('other.manual') ? 1 : 2;
        }
        // 实际退款金额如果没值的话,取明细中的退款金额
        if (itemLabel == window.vmI18n.t('form_label.actualRefundAmount')) {
          let total = 0;
          self.tableConfig.data.forEach((item) => {
    ...
    tableAddDetail() {
      ...
      for (let i = 0; i < this.reForm.config.length; i++) {
        ...
        if (
          ...
          window.vmI18n.t('form_label.originalOrderNo')
        ) {
      ...
      if (
        ...
        if (!self.isOne) {
          ...
          self.reForm.config.forEach((item) => {
            ...
            // 原始订单编号
            if (configItemLabel === self.vmI18n.t('form_label.originalOrderNo'))
              configItem.props.value = data.ID;
            // 店铺名称
            if (configItemLabel === self.vmI18n.t('table_label.shopName'))
              configItem.props.value = data.CP_C_SHOP_TITLE;
            // 原始平台单号
            if (
              configItemLabel === self.vmI18n.t('form_label.originalPlatformNo')
            )
              configItem.props.value = data.SOURCE_CODE;
            // 买家昵称
            if (configItemLabel === self.vmI18n.t('table_label.buyerNickname'))
            ...
            // 申请退款金额
            if (
              configItemLabel === self.vmI18n.t('form_label.apply_refundAmount')
            ) {
          ...
          self.reForm.config.find(
            (item) =>
              item.item.label == self.vmI18n.t('form_label.refundAmount') // 申请退款金额
        ...
        data.orderId = this.reForm.config.filter(
          (item) =>
            item.item.label == this.vmI18n.t('form_label.originalOrderNo')
        )[0].item.props.value;
        data.id =
          self.$route.params.customizedModuleId == 'New'
            ? -1
    ...
    async getDownUp() {
      ...
      if (res.data.code == 0) {
        ...
        childs.forEach((item) => {
          // '判责方'
          if (item.name === window.vmI18n.t('form_label.responsibleParty')) {
            dutyOfficer = item.combobox;
          }
          // '支付方式'
          if (item.name === window.vmI18n.t('table_label.paymentWay')) {
        ...
        self.reForm.config.forEach((item) => {
          ...
          // 判责方
          if (item.item.label == self.vmI18n.t('form_label.responsibleParty')) {
    ...
    sellerRemarkCompensation() {
      ...
      if (!this.sellerRemarkDataCreated) {
        config.forEach((configItem) => {
          ...
          switch (item.label) {
            case window.vmI18n.t('table_label.buyerNickname'):
              sellerRemarkData.USER_NICK = itemValue;
              break;
            case window.vmI18n.t('form_label.BuyerPhoneNumber'):
              sellerRemarkData.VIP_PHONE = itemValue;
              break;
            // '原始平台单号'
            case window.vmI18n.t('form_label.originalOrderNo'):
              sellerRemarkData.SOURCE_CODE = itemValue;
              break;
            // '退款金额'
            case window.vmI18n.t('form_label.refundAmount'):
              sellerRemarkData.AMT_RETURN_APPLY = itemValue;
              break;
            // '支付方式'
            case window.vmI18n.t('table_label.paymentWay'):
              sellerRemarkData.PAY_MODE = this.payTypeName(itemValue);
              break;
            // '支付账号'
            case window.vmI18n.t('form_label.payAccount'):
  ...
  computed: {
    ...
    applyAmt() {
      ...
      return this.reForm.config.filter(
        (item) => item.item.label == this.vmI18n.t('form_label.refundAmount')
      );
    },
  },
  watch: {
