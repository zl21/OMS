  ...
  mounted() {
    ...
    if (this.$route.query.id === '-1') {
        ...
        _this.service.orderCenter
          ...
          .then((res) => {
            if (res.data.code === 0) {
              ...
              _this.information.formValue.CP_C_PHY_WAREHOUSE_ID =
                this.warehouseId
              ...
              _this.information.formData[5].itemdata.valuedata =
                item.CP_C_SHOP_TITLE ? item.CP_C_SHOP_TITLE : ''
        ...
        this.service.orderCenter
          ...
          .then(async (res) => {
            ...
            queryList.forEach((subitem) => {
              ...
              newItem.QTY_REFUND =
                Number(subitem.QTY || 0) - Number(subitem.QTY_RETURN_APPLY || 0)
          ...
          .then(async (res) => {
            if (res.data.code == 0) {
              ...
              let checkFlag =
                ...
                [0, 2].includes(res.data.data.returnOrders.IS_TOWMS)
      ...
      this.information.formData =
        this.information.formData.concat(informationArr)
    }
  ...
  methods: {
    ...
    assignment(data) {
      ...
      this.information.formData[5].itemdata.valuedata = data.CP_C_SHOP_TITLE
        ...
        : ''
      this.information.formData[7].itemdata.valuedata =
        data.CP_C_LOGISTICS_ENAME ? data.CP_C_LOGISTICS_ENAME : ''
      ...
      this.replacement.formData[5].itemdata.valuedata =
        data.RECEIVER_PROVINCE_NAME ? data.RECEIVER_PROVINCE_NAME : ''
    ...
    labelClick(item, index) {
      ...
      if (index === 0) {
                      ...
                      on: {
                        'on-change': (e) => {
                          ...
                          params.row.AMT_REFUND = publicMethodsUtil.accMul(
                            e.target.value,
                            params.row.amt_refund_single == 0
                              ? params.row.PRICE ?? 0
              ...
              return h(
                'Select',
                {
                  ...
                  on: {
                    'on-change': (value) => {
                      console.log(params)
                      _this.jordanTableConfig.data[params.index].PRODUCT_MARK =
                        value
                    },
                  },
                },
        ...
        if (_this.returnSelectData.length) {
          ...
          _this.exchangeDtoList.data =
            _this.exchangeDtoList.data.concat(returnSelectDataArr)
          const newArr = []
                  ...
                  _this.service.common.screenresult(fromdata).then((res) => {
                    if (res.data.code === 0) {
                      ...
                      dimList.forEach((item) => {
                        // 商品编码
                        if (
                          item.label === _this.vmI18n.t('table_label.productNo')
                        ) {
              ...
              },
              {
                ...
                dimChange: (search) => {
                  _this.jordanTableConfig2.businessFormConfig.formValue.sku =
                    _this.jordanTableConfig2.businessFormConfig.formValue.sku.replace(
                      /(^\s*)|(\s*$)/g,
                      ''
                    )
                  // 模糊查询的方法
    ...
    saveData() {
      ...
      if (
        _this.isTowwms == 2 &&
        _this.$route.query.flag !== 'RefundToExchange'
      ) {
        // 传WMS成功状态的单据不可修改！
      ...
      if (
        _this.$route.query.id !== '-1' &&
        _this.$route.query.flag !== 'RefundToExchange'
      ) {
        if (
          ![20, 30, 50].includes(_this.status) ||
      ...
      // 是否原退
      _this.information.formValue.IS_BACK = Number(
        _this.information.formValue.IS_BACK
      )

      // 换货预留库存
      _this.information.formValue.IS_RESERVED = Number(
      ...
      let tempObj = {
        noDefectiveProductAllocation: 0,
        defectiveProductsNotAllocated: 1,
        defectiveProductsAllocated: 2,
      }

      Object.keys(tempObj).forEach((item) => {
        if (window.vmI18n.t(`other.${item}`) == _this.RESERVE_BIGINT07_type) {
          _this.information.formValue.STATUS_DEFECTIVE_TRANS = tempObj.item
        }
      })
      const Rlist = []
      let total = 0
      let item = _this.returnSelectData.length
        ? _this.returnSelectData
        : _this.refundDtoList.data
      for (let i = 0; i < item.length; i++) {
        ...
        Rlist.push({
          ...
          product_mark:
            item[i].PRODUCT_MARK == window.vmI18n.t('form_label.qualityGoods')
              ? '1'
              : '0', // '正品'
      ...
      for (let i = 0; i < Eitem.length; i++) {
        ...
        Elist.push({
          ...
          product_mark:
            Eitem[i].PRODUCT_MARK == window.vmI18n.t('form_label.qualityGoods')
              ? '1'
              : '0', // '正品'
      ...
      const params = {
        ...
        isRefund2Exchange:
          this.$route.query.flag === 'RefundToExchange' ? 1 : 0, // 如果未退货转换货订单,保存需传isRefund2Exchange
      }
      // 复制订单标识
            ...
            if (value.label === _this.vmI18n.t('form_label.reasonRefund')) {
              ...
              res.data.data.addcolums
                .find(
                  (item) =>
                    item.parentdesc &&
                    item.parentdesc == window.vmI18n.t('common.baseInformation')
                )
                .childs.forEach((item) => {
                  if (item.name == _this.vmI18n.t('form_label.reasonRefund')) {
                    arr = item.combobox
                  }
                })
              for (let i = 0; i < arr.length; i++) {
    ...
    getData(search, index) {
      ...
      this.service.common.skuQuery(param).then((res) => {
        if (res.data.code === 0 && index) {
          self.jordanTableConfig2.businessFormConfig.formData[1].AuotData =
    ...
    async entry(search, index) {
      ...
      this.service.common.skuQuery(param).then(async (res) => {
        if (res.data.code === 0) {
          ...
          if (index == 2) {
            ...
            if (y) {
              ...
              if (i) {
                ...
                // 无 => 吊牌价 * 数量
                y.AMT_REFUND = publicMethodsUtil.accMul(
                  y.amtRefundSingle || true ? obj.PRICE : y.amtRefundSingle,
                  obj.QTY_EXCHANGE
                )
          ...
          self[`${index == 1 ? 'amountReturned' : 'exchangeAmount'}`] = self
            .calculateMoney(dataList, 1)
            .toFixed(2)
          self.returnTotal()
    ...
    returnCancel(e) {
      this.returnSelectData = e
      this.amountReturned = this.calculateMoney(
        e.length > 0 ? this.returnSelectData : this.refundDtoList.data,
        1
      ).toFixed(2)
      this.returnTotal()
    },
      ...
      if ((_this.exchangeSelectData.length ||= true)) {
        ...
        for (let i = 0; i < item.length; i++) {
          if (
            ...
            _this.returnSelectData[0].PS_C_SKU_ECODE === item[i].PS_C_SKU_ECODE
          ) {
    ...
    async queryBounced(num) {
      ...
      if (
        (lists.ID ||= true) &&
        (lists.BILL_NO ||= true) &&
        (lists.source_code ||= true) &&
        (lists.receiver_name || true) &&
        (lists.user_nick || true) &&
        (lists.receiver_mobile || true) &&
        (lists.cp_c_store_ename || true) &&
        num == undefined
      ) {
    ...
    async queryorder(listData, isEnter) {
      ...
      this.information.formValue.CP_C_LOGISTICS_ID =
        this.onSelectData[0].CP_C_LOGISTICS_ID
      this.information.formValue.CP_C_LOGISTICS_ENAME =
        this.onSelectData[0].CP_C_LOGISTICS_ENAME
      ...
      _this.information.formData[14].itemdata.pid = this.warehouseId
      _this.information.formData[14].itemdata.valuedata =
        this.onSelectData[0].CP_C_PHY_WAREHOUSE_ENAME
      ...
      this.information.formValue.ORIG_SOURCE_CODE =
        this.onSelectData[0].SOURCE_CODE
      this.information.formValue.CP_C_SHOP_TITLE =
        this.onSelectData[0].CP_C_SHOP_TITLE
      this.information.formValue.CP_C_SHOP_ID =
        this.onSelectData[0].CP_C_SHOP_ID
      this.replacement.formValue.RECEIVE_ADDRESS =
        this.onSelectData[0].RECEIVER_ADDRESS // 收货人地址
      this.replacement.formValue.RECEIVE_NAME =
        this.onSelectData[0].RECEIVER_NAME // 收货人姓名
      this.replacement.formValue.RECEIVE_MOBILE =
        this.onSelectData[0].RECEIVER_MOBILE // 收货人电话
      ...
      replace.receiver_province_id =
        this.onSelectData[0].CP_C_REGION_PROVINCE_ID // 省id
      replace.receiver_province_name =
        this.onSelectData[0].CP_C_REGION_PROVINCE_ENAME // 收货人省份
      ...
      // 展示
      this.information.formData[5].itemdata.valuedata =
        this.onSelectData[0].CP_C_SHOP_TITLE // 店铺名称
      this.replacement.formData[5].itemdata.valuedata =
        this.onSelectData[0].CP_C_REGION_PROVINCE_ENAME // 省
      this.replacement.formData[6].itemdata.valuedata =
        this.onSelectData[0].CP_C_REGION_CITY_ENAME // 市
      this.replacement.formData[7].itemdata.valuedata =
        this.onSelectData[0].CP_C_REGION_AREA_ENAME // 区
      this.information.formData.forEach((item) => {
        // 是否原退
        if (
          item.label === this.vmI18n.t('form_label.whether_returned') &&
          this.information.formValue.IS_BACK
        ) {
          this.information.formValue.LOGISTICS_CODE =
            _this.onSelectData[0].EXPRESSCODE
        }
      })
      const queryList = []
      _this.addSelection = []
      _this.returnDetailAddTable.table.data = _this.addSelection
      for await (const selection of this.onSelectData[0]
        ...
        if (_this.jordanTableConfig.data.length) {
          val.forEach((item) => {
            ...
            if (d) {
              ...
              _this.jordanTableConfig.data =
                _this.jordanTableConfig.data.concat(arr)
            }
          })
        } else {
          _this.jordanTableConfig.data =
            _this.jordanTableConfig.data.concat(val)
        ...
        if (_this.jordanTableConfig2.data.length) {
          val.forEach((item) => {
            ...
            if (d) {
              ...
              arr.push(item)
              _this.exchangeDtoList.data =
                _this.exchangeDtoList.data.concat(arr)
              _this.jordanTableConfig2.data =
                _this.jordanTableConfig2.data.concat(arr)
            }
          })
        } else {
          _this.jordanTableConfig2.data =
            _this.jordanTableConfig2.data.concat(val)
    ...
    async reconstructionGetDetail(item, returnItem, ecode = '') {
      ...
      returnItem.clrList =
        ecode != '' ? this.clrListArr : item.selected.psCSpec1objList
      returnItem.sizeList =
        ecode != '' ? this.sizeListArr : item.selected.psCSpec2objList
      // return returnItem;
    },
