...
export default {
                      ...
                      h(
                        ...
                        [
                          h(
                            'span',
                            {},
                            `${item.PS_C_PRO_ECODE},${item.PS_C_CLR_ENAME},${item.PS_C_SIZE_ENAME},${item.PS_C_SKU_ECODE}`
                          ),
                          h(
                            'div',
                            {
  ...
  methods: {
    ...
    async queryBounced() {
      ...
      if (lists.tid) param.tid = lists.tid;
      ...
      try {
        const { data } = await this.service.orderCenter.searchButtonsInDetail(
          param
        );
    ...
    async okClick() {
      ...
      const {
        data,
      } = await this.service.orderCenter.manualMatchingConfirmationButton(
        param
      );
      if (data.code == 0) {
        _this.$parent.$parent.$parent.returnArr(
          data.data.oc_b_refund_in_id || ''
        );
    ...
    async okClick2() {
      const _this = this;
      if (
        _this.selectData[0].PRODUCTITEMS.length !== 1 &&
        _this.selectData[0].PRODUCTITEMS !== undefined
      ) {
        ...
        const { data } = await this.service.orderCenter.seachForced(param);
        if (data.code == 0) {
          _this.$parent.$parent.$parent.returnArr1(
            _this.selectData[0].ID,
            data.data.PS_C_SKU_ECODE_ACTUAL,
            _this.selectData[0].PRODUCTITEMS[0].ID
          );
          // _this.$parent.$parent.$parent.getList();
          _this.$parent.$parent.closeConfirm();
        } else {
    ...
    async wrongForce() {
      ...
      this.$parent.$parent.closeConfirm();
      if (data.code == 0) {
        this.$parent.$parent.$parent.returnArr1(
          data.data.returnId,
          data.data.PS_C_SKU_ECODE_ACTUAL,
          this.wrongSelectData[0].ID
        );
      } else {
    ...
    window.addEventListener('keydown', (e) => {
      ...
      if (key == 13) {
        if (_this.order.table.data.length && _this.componentData.type == 2) {
          ...
          _this.order.table.data.length &&
          _this.componentData.type == 3
        ) {
          _this.okClick2();
        }
      } else if (key == 27) {
