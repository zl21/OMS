...
export default {
    ...
    return {
      ...
      downLoadFormConfig: {
        ...
        formData: [
          ...
          {
            style: 'input', // 输入框类型
            label: $i18n.t('form_label.commodityID'), // 商品ID 输入框前文字
            value: 'shopID', // 输入框的值
  ...
  methods: {
    ...
    async download() {
      ...
      if (!self.downLoadFormConfig.formData[0].itemdata.pid) return self.$Message.warning($i18n.t('modalTips.be'));// 请选择需要下载的店铺
      if (formValue.shopID === '' && !self.timeConfig.data[0]) return self.$Message.warning($i18n.t('modalTips.bf'));// 商品ID和时间必填其中一个
      const param = {
