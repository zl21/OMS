...
export default {
  ...
  mounted() {
    ...
    node.ondrop = () => {
      ...
      this.service.orderCenter.saveQueryListConfig(param).then(() => {
  ...
  methods: {
    ...
    saveDragData() {
      ...
      if (self.childArr.length !== 0) {
        ...
        this.service.orderCenter.saveQueryListConfig(param).then(res => {
          if (res.data.code === 0) {
    ...
    getData() {
      ...
      this.service.orderCenter.queryListConfig(fromdata).then(res => {
        const formArr = [];
