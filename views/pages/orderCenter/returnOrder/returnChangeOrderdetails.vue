 ...
 * @LastEditTime: 2021-06-07 10:24:00
...
import { addDetailModalTableColumns, tuiColumns, huanColumns } from './returnConfig.js';
...
export default {
  ...
  methods: {
    ...
    async getReplaceData(objid) {
      ...
      } = await this.service.orderCenter.getALlOrderReturnAndItemInfo({ ID: objid });
      ...
      // 退款金额
      // let returnAmount = {
      //   PRO_ACTUAL_AMT:this.$OMS2.omsUtils.floatNumber(Number(OC_B_RETURN_ORDER.PRO_ACTUAL_AMT)),
      //   PRO_REAL_AMT:this.$OMS2.omsUtils.floatNumber(Number(OC_B_RETURN_ORDER.PRO_REAL_AMT)),
      //   SHIP_AMT:this.$OMS2.omsUtils.floatNumber(Number(OC_B_RETURN_ORDER.SHIP_AMT)),
      //   ADJUST_AMT:this.$OMS2.omsUtils.floatNumber(Number(OC_B_RETURN_ORDER.ADJUST_AMT)),
      //   EXCHANGE_AMOUNT:this.$OMS2.omsUtils.floatNumber(Number(OC_B_RETURN_ORDER.PRO_REAL_AMT)),
      //   FINAL_ACTUAL_AMT:this.$OMS2.omsUtils.floatNumber(Number(OC_B_RETURN_ORDER.FINAL_ACTUAL_AMT)),
      //   FINAL_REAL_AMT:this.$OMS2.omsUtils.floatNumber(Number(OC_B_RETURN_ORDER.FINAL_REAL_AMT))
      // }
      // R3.store.commit(`customize/returnAmount`, JSON.parse(JSON.stringify(returnAmount)));
      if (code === 0) {
        // 初始赋值
        ...
        this.businessActionTable.data = this.$parent.$parent.panelRef === '退货明细' ? OC_B_RETURN_ORDER_REFUND_ITEMS : OC_B_RETURN_ORDER_EXCHANGE_ITEMS; // 数据
        ...
        R3.store.commit('customize/returnOrderChangeItem', JSON.parse(JSON.stringify(this.toMainData)));
        this.totalNum()
      }
    },
    // 获取SKU数据
    async getPlaceData(page = 0, pageSize = 10) {
      ...
      let searchdata = { "table": "PS_C_SKU", "startindex": page, "range": pageSize, "fixedcolumns": fixedcolumns, "column_include_uicontroller": true, "isolr": false };
      ...
      if (code === 0) {
        // 处理表头和数据
        let columns = data.tabth.map(element => ({ title: `${element.name}`, key: `${element.colname}`, dataAcessKey: `${element.colname}` }));
    ...
    async getDetailModal() {
      const param = {
        ...
        SOURCE_CODE: this.$route.query.SOURCE_CODE ? this.$route.query.SOURCE_CODE : this.mainData.SOURCE_CODE,
      };
      this.loading = true;
      const {
        data: { code, data },
          ...
          return h('InputNumber', {
            ...
            on: {
              'on-change': e => {
                ...
                params.row.REFUND_FEE = this.$OMS2.omsUtils.floatNumber(Number(e) * Number(params.row.AMT_REFUND_SINGLE), 2);
                this.businessActionTable.data[params.index] = params.row;
                this.totalNum();
              }
            }
          ...
          return h('InputNumber', {
            ...
            on: {
              'on-change': e => {
                ...
                params.row.AMT_EXCHANGE = this.$OMS2.omsUtils.floatNumber(Number(e) * Number(params.row.PRICE_ACTUAL), 2);
                if (this.$route.params.customizedModuleId !== 'New') {
                  this.toMainData[this.$parent.$parent.panelRef === '退货明细' ? 'tui' : 'huan'][params.index] = params.row;
                  R3.store.commit('customize/returnOrderChangeItem', JSON.parse(JSON.stringify(this.toMainData)));
                }
          ...
          return h('InputNumber', {
            ...
            on: {
              'on-change': e => {
                ...
                params.row.AMT_EXCHANGE = this.$OMS2.omsUtils.floatNumber(Number(e) * Number(params.row.QTY_EXCHANGE), 2);
                this.businessActionTable.data[params.index] = params.row;
                if (this.$route.params.customizedModuleId !== 'New') {
                  this.toMainData[this.$parent.$parent.panelRef === '退货明细' ? 'tui' : 'huan'][params.index] = params.row;
                  R3.store.commit('customize/returnOrderChangeItem', JSON.parse(JSON.stringify(this.toMainData)));
    ...
    totalNum() {
      ...
      let amt = 0;
      let qty = 0;
      const key1 = (this.returnProduct == '0' || this.$parent.$parent.panelRef === '退货明细') ? 'QTY_REFUND' : 'QTY_EXCHANGE'; // 申请退货数量 : 换货数量
      const key2 = (this.returnProduct == '0' || this.$parent.$parent.panelRef === '退货明细') ? 'REFUND_FEE' : 'AMT_EXCHANGE'; // 退货金额 : 成交金额
      const key3 = (this.returnProduct == '0' || this.$parent.$parent.panelRef === '退货明细') ? 'PRO_ACTUAL_AMT' : 'EX_ACTUAL_AMT'; // 商品应退金额 : 换货金额
      ...
      setTimeout(() => {
        if (this.returnProduct == '0' || this.$parent.$parent.panelRef === '退货明细') {
        ...
        console.log(this.returnArr, this.changeArr)
        // 计算'商品应退金额'
        ...
        if (this.$parent.$parent.panelRef === '换货明细') {
          FINAL_ACTUAL_AMT = Number(returnAmount.PRO_REAL_AMT) + Number(returnAmount.SHIP_AMT) + Number(returnAmount.ADJUST_AMT) - Number(returnAmount.AMT_EXCHANGE);
          ...
          FINAL_ACTUAL_AMT = Number(returnAmount.PRO_REAL_AMT) + Number(returnAmount.SHIP_AMT) + Number(returnAmount.ADJUST_AMT);
    ...
    insertOrderDetail(dataArr = []) {
      ...
      const pryKey = this.returnProduct == '0' ? 'REFUND_ITEM_UNIQUE_KEY' : 'PS_C_SKU_ECODE';
      let pryKeyArr = [];
      if (!allDa.length) {
        // 当前为空，则直接新增
        data.forEach(it => { it.REFUND_FEE = this.$OMS2.omsUtils.floatNumber(it.QTY_REFUND * it.AMT_REFUND_SINGLE, 2); })
        self.businessActionTable.data = data;
        self.totalNum();
        return;
      }
      data.forEach((it) => {
        it.REFUND_FEE = this.$OMS2.omsUtils.floatNumber(it.QTY_REFUND * it.AMT_REFUND_SINGLE, 2);
        allDa.forEach((item) => {
          ...
          if (it[pryKey] == item[pryKey]) {
            ...
            if (item.RETURNABLE_QTY > sumQ) {
              ...
              item.REFUND_FEE = this.$OMS2.omsUtils.floatNumber((Util.accAdd(item.REFUND_FEE, it.REFUND_FEE), 2));
            } else {
              item.QTY_REFUND = it.RETURNABLE_QTY;
              console.log('REFUND_ITEM_UNIQUE_KEY:', it[pryKey], '数量大了/刚好！');
            }
    ...
    deleteMainTableData() {
      ...
      this.haveGift = '';
      this.haveGroup = '';
      selDa.forEach(it => { this.selectTogether(it); })
      ...
      if (this.returnProduct == '0') {
        ...
        if (haveGift) {
          ...
          msg = `请确认是否删除当前选中的退货商品，还存在关联的挂靠赠品：${haveGift.replace(/(，|,)$/, ' ')}，并且还存在组合/福袋下挂的其他关联商品：${haveGroup.replace(/，$/, '')}。`
        } else {
      ...
      this.$Modal.info({
        ...
        onOk: () => {
          this.$nextTick(() => {
            // 取差集展示：
            self.businessActionTable.data = this.$OMS2.omsUtils.getDifferentArr(allDa, selDa, key);
            this.totalNum();
            self.detailsArrData = [];
            if (this.$route.params.customizedModuleId === 'New') {
              this.toMainData[this.returnProduct == '0' ? 'tui' : 'huan'] = this.businessActionTable.data;
            } else {
              this.toMainData[this.$parent.$parent.panelRef === '退货明细' ? 'tui' : 'huan'] = this.businessActionTable.data;
            }
            this.$emit('subTableData', this.toMainData);
            R3.store.commit('customize/returnOrderChangeItem', JSON.parse(JSON.stringify(this.toMainData)));
    ...
    resetReturnMainTable() {
      ...
      this.$emit('subTableData', this.toMainData);
      this.tableConfig.data.forEach(it => { it._checked = false });
      this.indexL = [];
      R3.store.commit('customize/returnOrderChangeItem', JSON.parse(JSON.stringify(this.toMainData)));
    },
    detailAddCancel() {
    ...
    onSelectCancel(selection, row) {
      ...
      self.tableConfig.data.forEach((item, index) => { // 拿到ID去原数组中找
    ...
    async replaceOk() {
      ...
      } = await self.service.orderCenter.getReturnExchangeItemBySkuECode(params);
      // 获取商品明细
      if (data.OC_B_RETURN_ORDER_EXCHANGE_ITEMS === null) {
        return;
      }
      let newItem = data.OC_B_RETURN_ORDER_EXCHANGE_ITEMS[0];
      let findObj = tableData.find((item) => { return item.PS_C_SKU_ECODE === newItem.PS_C_SKU_ECODE })
      if (findObj) {
        let index = tableData.findIndex((item) => { return item.PS_C_SKU_ECODE === newItem.PS_C_SKU_ECODE });
    ...
    selectTogether(row) {
      ...
      allDa.forEach((it, index) => {
        if (it.REFUND_ITEM_UNIQUE_KEY == row.REFUND_ITEM_UNIQUE_KEY) this.indexL.push(index); // 记录当前选中行的index
